/* esri-leaflet - v2.3.3 - Wed Jan 29 2020 14:40:26 GMT-0600 (Central Standard Time)
...
var BasemapLayer = leaflet.TileLayer.extend({
  ...
  _initPane: function () {
    if (!this.__map.getPane(this.options.pane)) {
      var pane = this.__map.createPane(this.options.pane);
...
var Overlay = leaflet.ImageOverlay.extend({
  ...
  _reset: function () {
    if (this.__map.options.crs === leaflet.CRS.EPSG3857) {
      leaflet.ImageOverlay.prototype._reset.call(this);
    } else {
      leaflet.DomUtil.setPosition(this._image, this._topLeft.subtract(this.__map.getPixelOrigin()));
    }
  }
});
...
var RasterLayer = leaflet.Layer.extend({
  ...
  onAdd: function (map) {
    ...
    if (this._currentImage && this._currentImage._bounds.equals(this.__map.getBounds())) {
      map.addLayer(this._currentImage);
    } else if (this._currentImage) {
      this.__map.removeLayer(this._currentImage);
    ...
    if (this._popup) {
      this.__map.on('click', this._getPopupData, this);
      this.__map.on('dblclick', this._resetPopupState, this);
    }
  ...
  onRemove: function (map) {
    ...
    if (this._popup) {
      this.__map.off('click', this._getPopupData, this);
      this.__map.off('dblclick', this._resetPopupState, this);
    }

    this.__map.off('moveend', this._update, this);
  },
  ...
  unbindPopup: function () {
    if (this.__map) {
      this.__map.closePopup(this._popup);
      this.__map.off('click', this._getPopupData, this);
      this.__map.off('dblclick', this._resetPopupState, this);
    }
    this._popup = false;
    ...
    if (this.__map) {
      ...
      var onOverlayLoad = function (e) {
        ...
        if (this.__map) {
          ...
          if (newImage._bounds.equals(bounds) && newImage._bounds.equals(this.__map.getBounds())) {
            ...
            if (this.__map && this._currentImage.__map) {
              this._currentImage.setOpacity(this.options.opacity);
            } else {
              this._currentImage.__map.removeLayer(this._currentImage);
            }

            if (oldImage && this.__map) {
              this.__map.removeLayer(oldImage);
            }

            if (oldImage && oldImage.__map) {
              oldImage.__map.removeLayer(oldImage);
            }
          } else {
            this.__map.removeLayer(newImage);
          }
        }
  ...
  _update: function () {
    ...
    if (this.__map._panTransition && this.__map._panTransition._inProgress) {
      return;
    }
  ...
  _renderPopup: function (latlng, error, results, response) {
    ...
    if (this._shouldRenderPopup && this._lastClick.equals(latlng)) {
      ...
      if (content) {
        this._popup.setLatLng(latlng).setContent(content).openOn(this.__map);
      }
    }
  },
  ...
  _calculateBbox: function () {
    var pixelBounds = this.__map.getPixelBounds();

    var sw = this.__map.unproject(pixelBounds.getBottomLeft());
    var ne = this.__map.unproject(pixelBounds.getTopRight());

    var neProjected = this.__map.options.crs.project(ne);
    var swProjected = this.__map.options.crs.project(sw);
  ...
  _calculateImageSize: function () {
    ...
    var bounds = this.__map.getPixelBounds();
    var size = this.__map.getSize();

    var sw = this.__map.unproject(bounds.getBottomLeft());
    var ne = this.__map.unproject(bounds.getTopRight());

    var top = this.__map.latLngToLayerPoint(ne).y;
    var bottom = this.__map.latLngToLayerPoint(sw).y;
...
var ImageMapLayer = RasterLayer.extend({
  ...
  _buildExportParams: function () {
    var sr = parseInt(this.__map.options.crs.code.split(':')[1], 10);
...
var DynamicMapLayer = RasterLayer.extend({
  ...
  _getPopupData: function (e) {
    ...
    if (this.options.popup) {
      identifyRequest = this.options.popup.on(this.__map).at(e.latlng);
    } else {
      identifyRequest = this.identify().on(this.__map).at(e.latlng);
    }

    // remove extraneous vertices from response features if it has not already been done
    identifyRequest.params.maxAllowableOffset ? true : identifyRequest.simplify(this.__map, 0.5);
...
var FeatureManager = VirtualGrid.extend({
  ...
  _buildQuery: function (bounds) {
    ...
    if (this.options.simplifyFactor) {
      query.simplify(this.__map, this.options.simplifyFactor);
    }
...
var FeatureLayer = FeatureManager.extend({
  ...
  createLayers: function (features) {
    for (var i = features.length - 1; i >= 0; i--) {
      ...
      if (this._visibleZoom() && layer && !this.__map.hasLayer(layer) && (!this.options.timeField || this._featureWithinTimeRange(geojson))) {
  ...
  cellEnter: function (bounds, coords) {
    if (this._visibleZoom() && !this._zooming && this.__map) {
      leaflet.Util.requestAnimFrame(leaflet.Util.bind(function () {
    ...
    if (!this._zooming) {
      leaflet.Util.requestAnimFrame(leaflet.Util.bind(function () {
        if (this.__map) {
          ...
          var cellKey = this._cellCoordsToKey(coords);
          var layers = this._cache[cacheKey];
          var mapBounds = this.__map.getBounds();
          if (!this._activeCells[cellKey] && layers) {
            var removable = true;
  ...
  eachActiveFeature: function (fn, context) {
    ...
    if (this.__map) {
      var activeBounds = this.__map.getBounds();
      for (var i in this._layers) {

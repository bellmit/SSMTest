layui.use(['tree', 'util', 'jquery', 'form', 'laydate'], function () {
    ...
    function ClickHandler() {
        ...
        for (i = 0; i < gxnridList.length; i++) {
            var firstChar = gxnridList[i].slice("0", "1")
            firstCharList.push(firstChar)
            var isAllEqualFlag = isAllEqual(firstCharList)
            if (isAllEqualFlag == true) {
    ...
    function addLi(id, title) {
        ...
        if (id.startsWith("5")) {
            $(".addtree4").css('display', '');
            $("#node_add4").append("<li id=" + id + ">" + title + "</li>");
        }
    }
    ...
    function delLi(id) {
        ...
        var delFlag4 = document.querySelector('#node_add4').innerHTML;
        if (delFlag0 == '') {
        ...
        if (delFlag4 == '') {
            $(".addtree4").css('display', 'none');
        }

    }

    //监听select选择
    form.on('select(gxjd)', function (data) {
    ...
    function obtainGxywxx(gxywxxid) {
        $.ajax({
            ...
            success: function (res) {
                ...
                tree.setChecked('tree4', gxnrList);
            }
        });
    ...
    $('#save').on('click', function () {
        ...
        if (nameItem.length > 0) {
            for (var i = 0; i < nameItem.length; i++) {
                var firstChar = nameItem[i].slice("0", "1")
                nameItemList.push(firstChar)
                var isAllEqualFlag = isAllEqual(nameItemList)
            }
        }
        if (nameItem.length == 0) {
            layer.msg("请选择共享内容!");
            nameItem = []
        } else if (!isAllEqualFlag) {
            layer.msg("您已选择多个阶段的共享内容，请检查！")
            nameItem = []
        } else {
            $.ajax({
                type: 'POST',
                ...
                success: function (res) {
                    layer.msg("保存成功", {
                        time: 500,
                            ...
                            window.location = "sjgxjk-changzhou.html";
                        }
                    });
                }
            });
    ...
    $('#back').on('click', function () {
        var url = "sjgxjk-changzhou.html?loadPage=" + loadPage + "&loadSize=" + loadSize;
        window.location = url;
    });

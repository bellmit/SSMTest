layui.use(['element', 'layer', 'jquery', 'table', 'form', 'laypage',"laydate"], function () {
    ...
    function tableShow(page, size, data) {
        ...
        for (var i = 0; i < content.length; i++) {
            ...
            str += "<span style='float: left;width: 200px' data-index="+content[i].CHGCID+">项目代码：" + content[i].GCBH + "</span>";
            str += "<span style='float: left;width: 300px'>工程名称：" + content[i].GCMC + "</span>";
    ...
    $("#reset").on('click', function () {
        ...
        $("#gcdzxx").val('');
        $("#jsdwmc").val('');
        $(".gcdz select[name=gcdzqx] option:selected").val('');
        $(".gcdz select[name=gcdzs] option:selected").val('');
        $(".gcdz select[name=gcdzss] option:selected").val('');
        renderSData();

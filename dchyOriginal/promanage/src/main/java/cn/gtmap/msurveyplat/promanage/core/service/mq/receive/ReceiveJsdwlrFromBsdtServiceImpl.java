 ...
 * @description 接收来自办事大厅的建设单位注册信息
 * @time 2021/04/21 17:04
 */
@Service
public class ReceiveJsdwlrFromBsdtServiceImpl implements ChannelAwareMessageListener {
    ...
    @Override
    @RabbitListener(queues = Constants.BSDT_XMGL_JSDWLR_QUEUE)
    @Transactional(propagation = Propagation.REQUIRED)
    public void onMessage(Message message, Channel channel) {
        try {
            logger.info("**********来自办事大厅的建设单位注册信息--消息接收成功**********");
            logger.info(Constants.BSDT_XMGL_DIRECT_EXCHANGE + "-" + Constants.BSDT_XMGL_JSDWLR_QUEUE +
            ...
            logger.info("**********来自办事大厅的建设单位注册信息--消息接收失败**********" +
                    Constants.XMGL_BSDT_DIRECT_EXCHANGE + "-" + Constants.BSDT_XMGL_JSDWLR_QUEUE +
                    "消息接收时间为:" + CalendarUtil.getCurHMSStrDate());
    ...
    //统计分析数据推送
    @Transactional(propagation = Propagation.REQUIRED)
    public void saveJsdwzc(String str) {
        ...
        try {
            ...
            dchyXmglMqCzrz.setDldm(Constants.BSDT_XMGL_JSDWLR_QUEUE);
            dchyXmglMqCzrz.setDlmc(Constants.BSDT_XMGL_JSDWLR_QUEUE_MC);

            DchyXmglJsdwlrDto dchyXmglJsdwlrDto = JSONObject.parseObject(str, DchyXmglJsdwlrDto.class);

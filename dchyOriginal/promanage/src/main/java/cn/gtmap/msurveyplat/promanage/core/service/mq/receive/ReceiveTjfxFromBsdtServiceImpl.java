...
public class ReceiveTjfxFromBsdtServiceImpl implements ChannelAwareMessageListener {
    ...
    public void onMessage(Message message, Channel channel) {
        try {
            ...
            logger.info("**********线上统计分析数据--消息接收失败**********" +
                    Constants.XMGL_BSDT_DIRECT_EXCHANGE + "-" + Constants.BSDT_XMGL_TJFX_QUEUE +
                    "消息接收时间为:" + CalendarUtil.getCurHMSStrDate());
    ...
    //统计分析数据推送
    @Transactional(propagation = Propagation.REQUIRED)
    public void saveTjfx(String str) {
        ...
        try {
            ...
            dchyXmglMqCzrz.setDldm(Constants.BSDT_XMGL_TJFX_QUEUE);
            dchyXmglMqCzrz.setDlmc(Constants.BSDT_XMGL_TJFX_QUEUE_MC);
            DchyXmglTjfxDto dchyXmglTjfxDto = JSONObject.parseObject(str, DchyXmglTjfxDto.class);
            String key = dchyXmglTjfxDto.getKey();
            logger.info("*************************************输出*****************************************" + str);
            logger.info("*************************************输出*****************************************key:" + key);
            //返回结果塞进缓存
            EhcacheUtil.putDataToEhcache(key, dchyXmglTjfxDto);
        } catch (Exception e) {

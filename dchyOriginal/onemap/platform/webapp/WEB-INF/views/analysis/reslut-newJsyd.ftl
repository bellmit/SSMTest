...
<@aBase.tpl showHeader="false" css=cssContent bootVersion="-v3">
    ...
    <div class="container wrapper">
                    ...
                    <table>
                        ...
                        <#list result[0..] as item>
                            <tr>
                                <td>${item["DKBH"]!}</td>
                                <td>${item["DKMC"]!}</td>
                                <td>${item["xzjsydmj"]!?string(fixed)}</td>
                                <td>${item["ytjjsqmj"]!?string(fixed)}</td>
                            </tr>
                        </#list>
                    </table>
                </div>
        ...
        <div class="export-container text-center">
            <a class="btn btn-primary"
               data-excel='${excelData!}' onclick="exportExcel(this);">导出
                excel</a>
        </div>
    </div>
    ...
    <script type="text/javascript">
        ...
        function exportExcel(f) {
            ...
            openPostWindow("<@com.rootPath/>/geometryService/export/analysis", JSON.stringify(data), "xzjsyd_result.xml");
        }

...
<div class="container" style="width: 1240px;">
    <div class="row">
        <div style="float:left;margin: 20px">
            <h3 style="font-weight: normal; color:#188074"><span class="icon icon-columns"></span>&nbsp;地质灾害分析结果展示</h3>
        </div>
                ...
                <#list result?keys as key>
                    ...
                    <div id="${key!}" class="tab-pane fade in <#if key_index==0>active</#if>">
                        <table class="table-bordered">
                            ...
                            <tr>
                                <td rowspan="${detail?size-1}">${key!}</td>
                                <#list detail?keys as k>
                                    <#if k!="total">
                                        <#if k_index==0>
                                            <td>${k!}</td>
                                            <td>${detail[k]!}</td>
...
<script type="text/javascript">
    ...
    function exportExcel(){
        openPostWindow("<@com.rootPath/>/geometryService/export/analysis", result, "disas_result.xml");
    }

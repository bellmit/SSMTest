    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            function refreshData(startDateStr,endDateStr) {
                $.post("/omp/video/countCameraLog",{startDateStr:startDateStr,endDateStr:endDateStr,showParent:true},function (data) {
                    statisticsData(data);
                    fetchData(data);
                });
            }
            ...
            function statisticsData(data) {
                ...
                for(var groupName in dict){
                    ...
                    all["useFre"]+=dict[groupName]["useFre"]/5;
                }
                data["总共合计"] =all;
            }
            //判断是哪组
        ...
        <script id="cameraStatisticsTpl" type="x-tpl-mustache">
            <table style="border-collapse: collapse;" id="dataTable" border="1" cellspacing="0">
                <tr style="line-height: 40px;font-size: 20px;">
                    <th colspan="6">江苏省"慧眼守土"监控探头使用频率表</th>
                </tr>
                <tr>
                    <th style="width:16%">市</th>
                    <th style="width:16%">区县</th>
                    <th style="width:16%">探头数</th>
                    <th style="width:16%">使用频次</th>
                    <th style="width:26%">使用强度(次/个*天)</th>
                    <th style="width:%10">掉线数</th>
                </tr>
                {{#results}}
                <tr>

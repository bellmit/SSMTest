...
define(["dojo/_base/declare",
        ...
        "map/widgets/Insight/VideoOptions",
        "ztree",
    ...
    function (declare, arrayUtil, lang, topic, locale, parser, Draw, SpatialReference, GeometryService, BufferParameters, Point,
        ...
        var selectCamera;
        var widgetPath="/map/widgets/Insight";
        ...
        var instance, me = declare([BaseWidget], {
            ...
            onOpen: function () {
                ...
                if (VmOptions.checkUse) {
                    $.ajax({
                        url: root + "/insight/rest/unusedRecently",
                        success: function (r) {
                            if (r.length > 0) {
        ...
        function addOptionListeners() {
            ...
            $("#warnBtn").on('click', function () {
                ...
                $.ajax({
                    url: root + "/insight/queryWarnings",
        ...
        function bufferNearCameras(pnt){
            ...
            $.ajax({
                url:root+"/transitService/insight/rest/poi",
        ...
        function fetchVideoData() {
            ...
            if (VmOptions.videoStyle === VIDEO_STYLE_GROUP) {
                $.ajax(
                    {
                        url: root + "/insight/fetch/root/groups",
                        success: function (r) {
        ...
        function renderVideoPageData(page, size) {
            ...
            if (videoSearchCondition) {
                $.ajax({
                    url: root + "/insight/fetch/condition",
                    async: false,
        ...
        function addGraphicLayerListener() {
            graphicsLayer.on('click', function (evt) {
                debugger;
                var gra = evt.graphic;attr = gra.attributes;
        ...
        function onLeafRegionClick(event, treeId, treeNode) {
            if (!treeNode.isParent && !treeNode.hasOwnProperty("indexCode")) {
                $.ajax({
                    url: root + "/insight/fetch/region/cameras",
                                ...
                                selectCamera = layer.open({
                                    ...
                                    success: function () {
                                        ...
                                        $('.create').unbind('click').on('click', function () {
                                            ...
                                            $.ajax({
                                                url: root + "/insight/camera/view/scope",
                                                data: {indexCode: id},
                                                async: false,
                                                success: function (r) {
                                                    var videoData = r[0];
        ...
        function openProDetail(pro) {
            ...
            $(".preset-set").off('click').on('click', function () {
                ...
                $.getJSON(root + "/insight/preset/isCapture", function (rp) {
                    if (rp.result) {
        ...
        function quarterRender(year, season, proId, records) {
            ...
            if (esriLang.isDefined(__config.proWarn)) {
                try {
                    ...
                    $.getJSON(root + '/insight/recogs/origin', {ids: JSON.stringify(ids)}, function (res) {
                        if (lang.isArray(res)) {
                            //添加预警提示
                            console.log(res);
            ...
            $(".history-img-content").unbind('click').on('click', 'img', function () {
                ...
                if (type != undefined && type === "mp4") {
                    ...
                    layer.open({
                        ...
                        shade: 0,
                        area: ['640px', '320px'],
                        content: ['/omp/insight/mp4player?fileId=' + id, 'no']
                    });
                } else {
        ...
        function queryCameraByIndex(indexcode) {
            ...
            $.ajax({
                url: root + "/insight/query/indexcode/" + indexcode,
                async: false,
                success: function (r) {
                    if (r) {
                        camera = r;
        ...
        function startupVideo(type, camera, pNo) {
            ...
            if (typeof camera != "undefined" && ( typeof camera === "string" || typeof camera === "number" )) {
                for (var i = 0; i < videoList.length; i++) {
                    var video = videoList[i];
                    if (video.indexCode == (camera + '')) {
                        camera = video;
                        break;
                    }
                }
                //camera = queryCameraByIndex(camera + '');
            }
            if (typeof camera === "string") {
                ...
                if (lang.isArray(camera)) {
                    ...
                    if(true){
                        openPreviewWin(insightUrl,'camera','',camera.indexCode,'','preview');
                        return;
                    }
        ...
        function addNewPreset(pro, indexCode) {
            $.ajax({
                url: root + '/insight/preset/insert',
                method: 'post',
                data: {
                    id: pro.id,
                    proId: pro.proId,
        ...
        function deletePreset(pro, presetId) {
            $.ajax({
                url: root + '/insight/preset/delete',
                method: 'post',
                data: {
                    presetId: presetId,
                    proId: pro.proId
        ...
        function setEnabledPreset(data, presets, callback) {
            $.ajax({
                url: root + '/insight/preset/enabled',
                method: 'post',
                data: data,
                success: function (rp) {
                    if (rp.result) {
        ...
        function getSingleViewScope(priorityCamera, isSingle) {
            ...
            $.ajax({
                url: root + "/insight/camera/view/scope?ieRefresh=" + Math.random(),
            ...
            layer.open(
                {
                    ...
                    area: '800px',
                    shade: 0,
                    content: "<div style='width:800px;height:400px;'><img width='100%' height='100%'  src='" + __config.preWarningUrl + "/api/alarm/image?camera_id=" + indexCode + "&event_time=" + eventTime + "&token=" + __config.token + "'></img></div>",
                    success: function () {
                    }
                });
        }

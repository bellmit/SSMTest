...
public class VideoController extends BaseController {
    ...
    public String setCache(@PathVariable String token,
        ...
        try {
            if (StringUtils.isNotBlank(data)){ cacheManager.getCache(CACHE_TOKEN).put(token, data);}
            return token;
        } catch (Exception e) {
        ...
        try {
            ...
            if ((isNull(user) || SecHelper.isAdmin() || SecHelper.isGuest())&&loginName.isEmpty()) {
                ...
                if(loginName.isEmpty()) {
                    cameraList =videoMetadataService.getAll();
                }else {
                    cameraList =videoMetadataService.getAll(loginName);
                }
                //根据regionName去重
                TreeSet<Camera> cameraSet = new TreeSet<Camera>(new Comparator<Camera>() {
                    @Override
                ...
                if (cameraSet != null && cameraSet.size() > 0) {
                    for (Camera camera : cameraSet) {
                        if (StringUtils.isNotEmpty(camera.getRegionName())) {
                            names.add(camera.getRegionName());
                        }
                    }
                }
                List<CameraRegion> cameraRegions = videoMetadataService.findAllByNameIn(names);
                return cameraRegions;
            }
        } catch (Exception e) {
    ...
    public List<Camera> getCameraAll(@RequestParam(defaultValue = "0", required = false) int page,
        ...
        try {
            List<Camera> result;
            if(loginName.isEmpty()) {
                result =videoMetadataService.getAll();
            }else {
                result =videoMetadataService.getAll(loginName);
            }
            return result;
        } catch (Exception e) {
            throw new JSONMessageException(e.getLocalizedMessage());
    ...
    public void captureImg(@RequestParam(value = "guid") String guid,
        ...
        try {
            ...
            if (!imgFile.exists()) {imgFile = fileStoreService.createNewFile(guid + ".jpg");}
            try {
                file.transferTo(imgFile);
                logger.info("");
                logger.info("保存成功,");
...
//        videoManager.getVideoService().reloadConfig();
            ...
            if(map.containsKey("loginUserName")){
                ...
                if(user==null){
                    ...
                    if (StringUtils.isNotBlank(user.getUserId())){
                        logger.info("***获取用户所属部门***");
                        List<PfOrganVo> pfOrganVoList= sysUserService.getOrganListByUser(user.getUserId());
                        if (pfOrganVoList != null && !pfOrganVoList.isEmpty()){
                            PfOrganVo pfOrganVo = pfOrganVoList.get(0);
                            map.put("loginUserDept", pfOrganVo.getOrganName());
                        }
                    }
                }
                logger.info("***获取用户信息***");
    ...
    public Map searchLogs(@RequestParam(defaultValue = "1") int page, @RequestParam(defaultValue = "10") int size,
        ...
        try {
            ...
            if(cameraLogs!=null&&cameraLogs.getContent().size()>0){
                for(int i=0;i<cameraLogs.getContent().size();i++){
    ...
    /**
     * 描述：根据数据库日志表中用户id初始化用户所属部门
     *
     * @return
     * @author 杨文军
     * @create 10:42 2019/9/6
     **/
    @RequestMapping("/log/bindOrganByUserId")
    @ResponseBody
    public Map bindOrganByUserId(@RequestParam(defaultValue = "{}") String condition) {
        Map conditions = JSONObject.parseObject(condition, Map.class);
        String result = "初始化部门绑定失败!";
        try {
            ...
            for (CameraLog cameraLog : list) {
                if (StringUtils.isBlank(cameraLog.getUserDept())){
                    ...
                    cameraLoggerService.saveCameraLog(cameraLog);
                }
            }
            result = "初始化部门绑定成功!";
        } catch (Exception e) {
    ...
    public void exportLogs(@RequestParam(defaultValue = "{}") String condition, HttpServletResponse response) {
        Map conditions = JSONObject.parseObject(condition, Map.class);
        try {
            String exportOrganCameraLog = cameraLoggerService.getConstantValue(AppConfig.getProperty("exportOrganCameraLog"));
            String tpl = AppConfig.getProperty("exportExcelFileName");
            if(StringUtils.isNotBlank(exportOrganCameraLog) && StringUtils.isNotBlank(tpl)){
                HashMap result = Maps.newHashMap();
                //日志监控信息
                List<CameraLog> list = cameraLoggerService.export(conditions);
                result.put("list",list);
                //统计每个行政区登陆总次数
                String exportOrganCameraLogArray[] = exportOrganCameraLog.split("\\|");
                for (int index = 0; index < exportOrganCameraLogArray.length; index++){
                    List<Map> cameraLogCountByOrganList = cameraLoggerService.countOptCameraLogByOrgan(exportOrganCameraLogArray[index], conditions);
                    String key = "list_organ_" + index;
                    result.put(key,cameraLogCountByOrganList);
                }
                ExportExcelUtil.msExcelTpl2Response(ExportExcelUtil.getTplPath(tpl
                        , ""), "监控点日志信息.xls", result, response);
            }else {
                List<CameraLog> list = cameraLoggerService.export(conditions);
    ...
    public List queryWarning(@RequestParam(value = "requestUrl") String requestUrl,
        ...
        try {
            String indexCodesStr = StringUtils.join(indexCodes.toArray(), ",");
            HttpClient httpClient = new HttpClient();
            PostMethod httpMethod = new PostMethod(requestUrl);
            httpMethod.setRequestHeader("token", AppConfig.getProperty("alarm.token"));
            httpMethod.setParameter("cameraList", indexCodesStr);
            httpMethod.setParameter("beginTime", startTime);
            httpMethod.setParameter("endTime", nowTime);
            httpMethod.setParameter("beginIndex", String.valueOf(beginIndex));
            httpMethod.setParameter("loadSize", String.valueOf(loadSize));
            Object json = HttpRequest.request(httpClient, httpMethod, "json");
            logger.info("调取warnings返回信息为" + JSON.toJSONString(json));
            Map map = (Map) json;
            if (map.containsKey("alarmList")) {
                List<Map> list = (List<Map>) map.get("alarmList");
                if (list.size() > 0) {
                    warns.addAll(list);
                }
            }
        } catch (Exception e) {
            logger.error("调取alarm服务失败" + e.getLocalizedMessage());
        }
//        }
    ...
    public Map overrideProjImg(HttpSession session,
    	...
    	try {
    		...
    		if(oldFileStore!=null){
    			//保存分析结果
    			videoRecognizeService.saveRecogHikJiangyin(oldFileStore, diffXyJsonStr);
    		}
...
//     */
    ...
    /**
     * 描述：导出"慧眼守土"监控探头使用频率表
     *
     * @return
     * @author 杨文军
     * @create 15:48 2019/7/29
     **/
    @RequestMapping("/export")
    public void export(@RequestParam String startDateStr,@RequestParam String endDateStr,
                            HttpServletResponse response) throws Exception {
        try {
            Map result = countCameraLog(startDateStr,endDateStr,false);
            Workbook workbook = cameraLoggerService.getAllExportGroupByOrgan(result);
            response.setContentType("application/vnd.ms-excel");
            response.setHeader("Content-disposition", "attachment;filename=" + URLEncoder.encode("慧眼守土监控探头使用频率表.xls", Constant.UTF_8));
            OutputStream os = response.getOutputStream();
            workbook.write(os);
            os.flush();
            os.close();
    ...
    public String cameraStatistics(Model model){
        model.addAttribute("reportUrl", AppConfig.getProperty("report.url"));

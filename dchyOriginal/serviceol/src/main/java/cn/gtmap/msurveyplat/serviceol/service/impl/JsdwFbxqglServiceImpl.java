                        ...
                        if(StringUtils.isNotBlank(chxmid)){
                            ...
                            if(chxmOptional.isPresent()){
                                ...
                                if(StringUtils.isNotBlank(xmglChxmXsbf.getXmzt()) && StringUtils.equals(xmglChxmXsbf.getXmzt(),"99")){
                                    Date bjsj = xmglChxmXsbf.getBjsj();//办结时间
                                    mapTemp.put("DQJD","项目办结");//当前节点  -> 项目办结
                                    mapTemp.put("JDSJ",sdf.format(bjsj));//节点时间
                                }
                                else if(CollectionUtils.isNotEmpty(list)){/*成果是否提交*/
                                    Map<String, Object> cgMap = list.get(list.size() - 1);
                                    if(MapUtils.isNotEmpty(cgMap)){
                                        Date tjsj = (Date) cgMap.get("TJSJ");
                                        if(null != tjsj){
                                            mapTemp.put("DQJD","成果提交");//当前节点  --> 成果提交
                                            mapTemp.put("JDSJ",sdf.format(tjsj));//节点时间
                                        }
                                    }
                                }
                                else if(StringUtils.isNotBlank(xmglChxmXsbf.getXmzt()) && ! StringUtils.equals(xmglChxmXsbf.getXmzt(),"1")){
                                    Date slsj = xmglChxmXsbf.getSlsj();//备案时间
                                    Date basj = xmglChxmXsbf.getBasj();
                                    if(null != basj){
                                        mapTemp.put("DQJD","项目备案");//当前节点  -> 项目备案
                                        mapTemp.put("JDSJ",sdf.format(basj));//节点时间
                                    }
                                    else if(null != slsj){
                                        mapTemp.put("DQJD","项目备案");//当前节点  -> 项目备案
                                        mapTemp.put("JDSJ",sdf.format(slsj));//节点时间
                                    }
                                }
                            }
                        }

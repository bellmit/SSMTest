...
/**
 * @author <a href="mailto:lijun1@gtmap.cn">xiejianan</a>
 * @description 线上在线办结  线上-->线下库
 * @time 2020/11/27 15:04
 */
@Service
public class SendZxbjToXmglServiceImpl implements RabbitTemplate.ConfirmCallback {
    ...
    public void sendDirectMsg(String exchange, String routingKey, DchyXmglZxbjDto dchyXmglZxbjDto) {
        logger.info("**********在线办结数据--消息发送成功**********");
        logger.info("**********线上推送办结数据的请求数据:**********" + JSONObject.toJSONString(dchyXmglZxbjDto));
    ...
    public void confirm(CorrelationData correlationData, boolean ack, String cause) {
        if (ack) {
            ...
            logger.info("**********在线办结数据--消息发送失败**********" +
                    Constants.BSDT_XMGL_DIRECT_EXCHANGE + "-" + Constants.BSDT_XMGL_ZXBJ_QUEUE +
                    "消息发送时间为:" + CalendarUtil.getCurHMSStrDate());

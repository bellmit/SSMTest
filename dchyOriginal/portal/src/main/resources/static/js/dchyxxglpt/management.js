...
layui.use(['jquery', 'element', 'tree', 'util', 'form', 'laytpl', 'slider',], function () {
    var $ = layui.jquery
    ...
    $(function () {
        ...
        $('.layer-content').on('click', 'h3', function () {
            if ($(this).find('i').hasClass('layui-icon-triangle-r')) {
                $(this).find('i').removeClass('layui-icon-triangle-r').addClass('layui-icon-triangle-d');
            } else {
                $(this).find('i').addClass('layui-icon-triangle-r').removeClass('layui-icon-triangle-d');
            }
        ...
        slider.render({
            ...
            , setTips: function (value) { //自定义提示文本
                return '透明度：' + Number(value) / 100;
            }
            , change: function (value) {
                var val = value.substr(4,3);
                document.getElementById('mapImg').style.opacity = Number(val);
            }
        });
    })
});

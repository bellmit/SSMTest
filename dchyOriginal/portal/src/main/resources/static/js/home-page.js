    ...
    $(function () {
        ...
        function createWebsocket(userId) {
            ...
            if ('WebSocket' in window) {
                try {
                    websocket = new WebSocket('ws://' + location.origin.replace("http://", "") + currentOrigin + '/ws/yhxxtx');
                    init();
                } catch (e) {
                    reconnect();
                }
        ...
        function init() {
            ...
            websocket.onmessage = function (event) {
                ...
                var wdqsl = parseInt(res.data.wdqsl);
                if (wdqsl > 99) {
                    if ($(".message-tip").parents(".layui-nav-first").find("cite .message-tip").length) {
                        $(".message-tip").parents(".layui-nav-first").find("cite .message-tip").html('99+')
                    } else {
                        $(".message-tip").parents(".layui-nav-first").find("cite").append("<span class='message-tip'>99+</span>")
                    }
                    $(".message-tip").removeClass("bdc-hide")
                    $(".message-tip").html('99+');
                } else if (wdqsl) {
                    if ($(".message-tip").parents(".layui-nav-first").find("cite .message-tip").length) {
                        $(".message-tip").parents(".layui-nav-first").find("cite .message-tip").html(wdqsl)
                    } else {
                        $(".message-tip").parents(".layui-nav-first").find("cite").append("<span class='message-tip'>" + wdqsl + "</span>")
                    }
                    $(".message-tip").removeClass("bdc-hide")
                    $(".message-tip").html(wdqsl);
                }
                heartCheck.start();
            }
        ...
        $(".return-home").on("click", function () {
            $.ajax({
                type: "GET",
                url: getContextPath() + "/index/getUrl",
                async: false,
        ...
        $('.bdc-logout').on('click', function () {
            var logoutIndex = layer.confirm('您确定要退出吗？', {
                ...
                $.ajax({
                    type: "GET",
                    url: getContextPath(true) + "/portal/logout",
                    success: function (data) {
                    },
                    error: function (e) {
                    }
                ...
                $.ajax({
                    type: "GET",
                    url: getContextPath(true) + "/msurveyplat-promanage/logout",
                    success: function (data) {
                        window.location.href = getContextPath() + "/msurveyplat-promanage/#/";
                    },
                    error: function (e) {
                    }
                });
                $.ajax({
                    type: "GET",
                    url: getContextPath(true) + "/msurveyplat-server/logout",
                    success: function (data) {
                    },
                    error: function (e) {
                    }
                ...
                $.ajax({
                    type: "GET",
                    url: getContextPath(true) + "/msurveyplat-serviceol/logout",
                    success: function (data) {
                    },
                    error: function (e) {
                    }
                ...
                $.ajax({
                    type: "GET",
                    url: getContextPath(true) + "/portal-ol/logout",
                    success: function (data) {
                        window.location.href = getContextPath() + "/portal-ol/#/";
                    },
                    error: function (e) {
                    }
                });
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                // 判断是否管理员用户
                if (data.roleName && (data.roleName.indexOf("管理单位") > -1 || data.roleName.indexOf("系统管理员") > -1)) {
                    $(".edit-password").removeClass("bdc-hide")
                }
                // 侧边栏动态渲染
                $.ajax({
                    type: "GET",
                    url: getContextPath() + "/index/getSysMenu?systemId=" + systemId,
                    data: {},
                    success: function (data) {
                        var defaultUrl = function (_dataUrl) {
                            _dataUrl.forEach(function (item) {
                                item.url = item.url || (new Date().getDate() + Math.random(10000));
                                defaultUrl(item.childTree);
                            })
                        }
                        defaultUrl(data);
                        var mainUrl = function (_dataUrl) {
                            var url = "";
                            $.each(_dataUrl, function (i, item) {
                                if (null != item.childTree && undefined != item.childTree && item.childTree.length > 0) {
                                    url = mainUrl(item.childTree);
                                    return false;
                                } else {
                                    url = item.url;
                                    return false;
                        ...
                        $("iframe.layadmin-iframe").attr("src", mainUrl(data));
        ...
        //点击基本信息
        function getBaseInfo(arr, val) {
            if (arr && arr.length > 0) {
                $.each(arr, function (index, item) {
                    var dd = $(item).children('dl').children('dd');
                    if (dd.length > 0) {
                        getBaseInfo(dd, val);
                    } else {
                        if ($(item).children('.bdc-show-msg').text().trim() == val.trim()) {
                            $(item).parents('li').addClass('layui-nav-itemed').end().children('.bdc-show-msg').click();
                        }
                    }
                })
            }
        ...
        $('.bdc-update').on('click', function () {
            ...
            var $parentLi = $('.layui-side-menu .layui-nav-tree li');
            $parentLi.removeClass('layui-nav-itemed');
            getBaseInfo($parentLi, '修改密码');
        });

        //点击基本资料
        $('.bdc-msg-nav').on('click', function () {
            // $(this).parents('.bdc-person-child').removeClass('layui-show');
            var $parentLi = $('.layui-side-menu .layui-nav-tree li');
            $parentLi.removeClass('layui-nav-itemed');
            getBaseInfo($parentLi, '消息提醒');
        });
        ...
        //跳转成果管理系统
        $('#LAY-system-side-menu').on('click', '.bdc-open-new-window', function () {
        ...
        if (loadHome == 'true') {
            ...
            table.on('rowDouble(waitTableFilter)', function (obj) {
                ...
                window.open("../view/service-page.html?name=" + obj.data.taskId, "待办任务");
            });
            ...
            table.on('rowDouble(doneTableFilter)', function (obj) {
                ...
                sessionStorage.setItem('lcArray' + obj.data.taskId, JSON.stringify(lcArray));
                window.open("../view/service-page.html?name=" + obj.data.taskId, obj.data.processDefName);
            });
            ...
            table.on('tool(doneTableFilter)', function (obj) { //bdcdyhList为table的lay-filter对应的值
                if (obj.event === 'openpage') {
                    ...
                    sessionStorage.setItem('lcArray' + obj.data.taskId, JSON.stringify(lcArray));
                    window.open("../view/service-page.html?name=" + obj.data.taskId, obj.data.text1);
                }
            });
            // 监听项目列表 行双击
            table.on('rowDouble(listTableFilter)', function (obj) {
                ...
                window.open(getContextPath() + "../view/service-page.html?name=" + obj.data.procInsId, obj.data.text1);
            });
            ...
            table.on('tool(listTableFilter)', function (obj) { //bdcdyhList为table的lay-filter对应的值
                if (obj.event === 'openpage') {
                    ...
                    sessionStorage.setItem('listArray' + obj.data.procInsId, JSON.stringify(listArray));
                    window.open("../view/service-page.html?name=" + obj.data.procInsId, obj.data.text1);
                }
            });
    ...
    function renderRlTable(tableId, currentId, toolbar) {
        table.render({
            ...
            cols: [
                [
                    {type: 'checkbox', width: 50, fixed: 'left'},
                    {field: 'text1', minWidth: 110, title: '受理编号'},
                    {field: 'text2', minWidth: 270, templet: '#bdcdyhTpl', title: '不动产单元号'},
                    {field: 'text3', minWidth: 100, title: '权利人'},
                    {field: 'text5', minWidth: 100, title: '义务人'},
                    {field: 'text4', minWidth: 200, title: '坐落'},

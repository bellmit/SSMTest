                ...
                <div id="Summary" class="tab-pane fade in">
                    <div class="well well-lg summary">
                        <#if result??>
                            <ol>
                                <#if ogArea??>
                                    <li>本地块面积 <span class="number">${(ogArea!)?string(fixed)}</span>m<sup>2</sup></li>
                                </#if>
                                <#list result?keys as key>
...
<script type="text/javascript">
    ...
    function openPostWindow(url, data, fileName, fileType) {
        if (data === null || data === 'null' || data === undefined)
            ...
            //控制哈尔滨版本,去掉规划分析结果中的 规划基本农田调整和 重点建设项目。
            if (activeId === 'gh') {
                if (data["规划基本农田调整"] !== undefined && data["重点建设项目"] !== undefined) {
                    delete data["规划基本农田调整"];
                    delete data["重点建设项目"];
                }
            }

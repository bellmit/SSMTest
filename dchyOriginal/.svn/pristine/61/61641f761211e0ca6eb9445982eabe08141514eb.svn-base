    ...
    */
    ...
    function ImageViewer(container, options) {
        ...
        if(options.showClose){
            container.append('<div class="iv-close"></div>');
            self.closeHandler=container.find('.iv-close');
        }
    ...
    ImageViewer.prototype = {
        ...
        _init: function () {
            ...
            imageWrap.on("mousewheel" + eventSuffix + " DOMMouseScroll" + eventSuffix, function (e) {
                ...
                if(!viewer.options.isCanvas) {
                    viewer.zoom(zoomValue, {
                        x: x,
                        y: y
                    });
                }else{
                    viewer.zoom(zoomValue, {  //放大后重绘图片
                        x: x,
                        y: y
                    }, drawImage);
                }

                function drawImage(imgWidth, imgHeight) {
                    // console.log("imgWidth:"+imgWidth+", imgHeight:" + imgHeight);
                    if(imgWidth!=NaN && imgHeight!=NaN){
                        var image = new Image();
                        image.src = viewer.currentImg.data("src");
                        viewer.currentImg[0].height = imgHeight;
                        viewer.currentImg[0].width = imgWidth;
                        settings.canvasContext.clearRect(0, 0, imgWidth, imgHeight);
                        settings.canvasContext.drawImage(image, 0, 0, imgWidth, imgHeight);
                        image = null;
                    }
                }
            ...
            if (viewer._fullPage) {
                ...
                container.find('.iv-close').on('click', function () {
                    console.log(viewer.options);
                    viewer.hide();
                    viewer.options.onClose.call();
                });
            }
        },

        //method to zoom images
        zoom: function (perc, point, callback) {
            ...
            function zoom() {
                ...
                if(callback) callback(imgWidth, imgHeight);
        ...
        _calculateDimensions: function () {
            ...
            var self = this,
                ...
                if(this.options.isCanvas){
                    var imageWidth = curImg[0].width,
                    imageHeight = curImg[0].height;
                }else{
                    var imageWidth = curImg.width(),
            ...
            if(this.options.isCanvas){
                curImg[0].width = imgWidth ;
                curImg[0].height = imgHeight;
        ...
        load: function (image, hiResImg) {
            ...
            //增加canvas图片展示
            if(this.options.isCanvas){
                snapImageWrap.prepend('<canvas class="iv-snap-image"   data-original="'+image+'"></canvas>');

                if(image)
                    this.imageWrap.prepend('<canvas class="iv-large-image" data-original="'+image+'"></canvas>');
                else
                    this.imageWrap.append('<canvas class="iv-large-image" data-original="' + hiResImg + '"></canvas>');
            }else{
                snapImageWrap.prepend('<img class="iv-snap-image"   data-original="'+image+'"/>');

                if (hiResImg) {
                    this.imageWrap.append('<img class="iv-large-image" src="' + hiResImg + '"  data-original="' + hiResImg + '" />')
                }
                else if(image){
                    this.imageWrap.prepend('<img class="iv-large-image" data-original="'+image+'"/>');
                }
    ...
    window.ImageViewer = function (container, options) {
        ...
        container = $(container);
        if (container.is('img') || container.is('canvas')) {
            imgElm = container;
            imgSrc = imgElm[0].src || imgElm.attr('data-src');

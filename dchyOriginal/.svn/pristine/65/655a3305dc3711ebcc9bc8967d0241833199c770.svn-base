...
function pjqLogin() {
    ...
    var level = Number(5);
    var motto ="用心服务";
    var playtime = Number(5);
    var photoData ;
    $.ajax({
        url: getContextPath() + "/rest/v1.0/pjq/userInfo",
        type: "GET",
        dataType: "json",
        success: function (data) {
            userData = data;
             ...
             placeNo = userData.placeNo;
             photoData = userData.base64;
             if(photoData != ""){
                 //带照片登录
                 var res = HidHelperUtils.LoginWithPhoto(name, no, placename, placeNo, level, motto, playtime, photoData);
                 if(res != 0){
                     layer.msg("登陆评价器失败！请检查！");
                 }
                console.log("res"+res);
             } else {
                 var res1 = HidHelperUtils.Login(name, no, placename, placeNo, level, motto, playtime);
...
function ShowInformationPjq(jhxx) {
    /*var htmlTextBox = document.getElementById("ph");
    if (htmlTextBox && htmlTextBox.value) {
        var html = htmlTextBox.value;

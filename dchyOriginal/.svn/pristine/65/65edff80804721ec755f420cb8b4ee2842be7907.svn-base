...
define(["dojo/_base/declare",
    ...
    function _init() {
        ...
        commonAnalysis = new BaseAnalysis(BaseAnalysis.TYPE_CUSTOM_COMMON, _map);
        commonAnalysis.setAnalysisMethod("bljbnt")
        commonAnalysis.setDataSource(_analysisConfig.dataSource);
        commonAnalysis.setAnalysisLyrs(_analysisConfig.layers);
        ...
        if (_analysisConfig.responseType) {
            ...
            topic.subscribe("anaResultHandler", function (data) {
                ...
                addResultHandlerListener(_map, result.excelData, result.excelList);
            });
        }
        ...
        $("#jbntAnalysisTitle").append(_label);
        $("#jbntAnalysisPanel").append(renderTpl(baseTpl, option));
    ...
    function onShareData(data) {
        if (data != undefined) {
            ...
            for (var i = 0; i < shareResult.length; i++) {
                ...
                commonAnalysis.addGraphic(item);
            }
        }
    }

<template>
    <div class="table upload-table">
        <form id="uploadForm">
            <input type="file" class="bdc-hide" multiple :id="uploadId"/>
        </form>
                ...
                <template v-for="(item,index) in data">
                    <tr data-index="N" class="data-type-zone" :key="index">
                        ...
                        <td data-field="attachmentName" v-for="(col) in columns" :key="col.field" :style="{'width': col.width || 'auto'}" align="center">
                            <div data-field="5" align="center" data-content="" v-if="col.type == 'operation'" class="position-relative layui-table-cell">
                                <span :class="item.disabled ? 'table-btn-a table-btn-disabled cursor-not-allowed':'table-btn-a'" v-if="!readonly" @click="uploadAttachment(item,index)">上传</span>
...
export default {
    props: {
        ...
        uploadId: {
            type: String,
            default: "attachmentUploadFileInput"
        }
    },
    data() {
        return {
    ...
    methods: {
        uploadAttachment(item,index){
            ...
            if((!item.CHDWID || !item.CHDWID.length)&&!this.projectInfoData.chdwId&&this.check){
                layer.msg("请先选择测绘单位")
                return
            }
        ...
        downloadFj(item,i){
            if(!item.WJZXID){
                ...
                location.href=location.origin + '/msurveyplat-promanage/fileoperation/download?wjzxid=' +item.WJZXID
            }
        },
        addNew(){
            this.$emit("tooladd")

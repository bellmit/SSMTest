                ...
                <Row>
                    ...
                    <i-col span="6">
                        <FormItem label="操作类型" style="width: 100%">
                            <Select filterable clearable v-model="recordForm.czlx">
                                <Option v-for="(czlx,index) in czlxList" :key="index" :value="czlx.DM">{{czlx.MC}}</Option>
                            </Select>
                        </FormItem>
                    </i-col>
                    <i-col span="8">
    ...
    methods: {
        getDictInfo(){
            ...
            getDictInfo(params).then(res => {
                this.czlxList = res.data.dataList || [];
                this.czlxList.unshift({
                    MC: "全部",
                    DM: ""
                })
            })

    ...
    <#if value.total &gt;0||number&gt;0>
        <#switch fid>
            ...
            合计<span class="number">${value.total!0}</span>m<sup>2</sup>.
                <#break/>
            <#case "gh">
        本地块与${value.alias!}图层叠加，压盖土地用途区<span class="number">${value.tdytq!0}</span>m<sup>2</sup>,压盖建设用地管制区<span class="number">${value.jsydgzq!0}</span>m<sup>2</sup>,
        压盖规划基本农田调整<span class="number">${value.ghjbnttz!0}</span>m<sup>2</sup>,压盖重点建设项目<span class="number">${value.zdjsxm!0}</span>m<sup>2</sup>,合计<span class="number">${value.total!0}</span>m<sup>2</sup>
            <#break/>
            <#case "bp">
        ...
        <div class="span12">
            <div id="resultContainer" class="tab-content large">
                <#list result?keys as key>
                    ...
                    <div id="${key!}" class="tab-pane fade in <#if key_index==0>active</#if>" data-exceldata='${item.excelData!}'>
                        ${item.tpl!}
                    </div>
                </#list>
...
<script type="text/javascript">
    ...
    function gotoLocation(geojson, key) {

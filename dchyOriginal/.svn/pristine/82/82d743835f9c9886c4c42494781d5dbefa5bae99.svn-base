        ...
        <div>
            <div v-for="(item,index) in adviceList" :key="item.ANSWERID" class="advice-item margin-top-10">
                <div class="advice-title">
                    <div v-if="item.ISOPEN=='1'"><div class="zt-gk advice-zt"><Icon type="ios-mail-open-outline" style="font-size: 16px;margin-right:5px"/>公开</div>{{item.TITLE}}</div>
                    <div v-else><div class="zt-gk advice-zt"><Icon type="ios-mail-outline"  style="font-size: 18px;margin-right:5px"/>非公开</div>{{item.TITLE}}</div>
...
<script>
import { queryAdminAdviceList } from "../../../service/advice"
...
export default {
    ...
    methods: {
        ...
        getAdviceList(page,size){
            ...
            queryAdminAdviceList(this.formInline).then(res => {
                this.$loading.close();

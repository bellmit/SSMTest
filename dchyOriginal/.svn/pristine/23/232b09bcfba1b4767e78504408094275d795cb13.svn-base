...
import { queryEditRecordList } from "../../../service/mlk"
export default {
    data() {
        return {
            formInline: {
    ...
    methods: {
        ...
        getRecordList(page,size){
            ...
            this.formInline.glsxid = this.mlkid;
            ...
            queryEditRecordList(this.formInline).then(res => {
                ...
                res.data.dataList.forEach((item) => {
                    item.CHDWMC = this.$route.query.dwmc
                })
        ...
        // 查看详情
        viewDetail(item){
            this.$router.push({path: "/survey/edit/record/detail",query: {rzid: item.RZID}})
        }
    },
}
</script>

...
import cn.gtmap.msurveyplat.promanage.service.impl.ExportPdfServiceImpl;
...
//            new PdfReport().generatePDF(document);
    ...
    static {
        try {
            // 获取路径地址
            modelPath = AppConfig.getProperty("egov.conf") + "/promanage/model.pdf";
...
//        table.addCell(createCell("美好的一天", headfont, Element.ALIGN_LEFT, 6, false));
        table.addCell(createCell("回执单", textfont, Element.ALIGN_CENTER, 4));

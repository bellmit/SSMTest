...
define(["dojo/_base/declare",
    ...
    "css!static/js/map/template/analysis/analysis.css"], function (declare, lang, arrayUtil, topic, RandomUuid, Handlebars, Graphic, Polygon,Color, MapUtils, BaseWidget,
                                                                   JsonConverters, BaseAnalysis, GeoDataStore, GeometryIO, MapPopup, EsriSymbolsCreator, baseTpl, listItem, resultTpl) {
    ...
    function _init() {
        ...
        ghscAnalysis = new BaseAnalysis(BaseAnalysis.TYPE_GHSC, _map);
        ...
        if (_ghscConfig.hasOwnProperty("fillColor")) {
            fillColor=_ghscConfig.fillColor;
        }
        if (_ghscConfig.hasOwnProperty("locFillColor")) {
            locFillColor=_ghscConfig.locFillColor;
        }
        if (_ghscConfig.hasOwnProperty("lineColor")) {
            lineColor=_ghscConfig.lineColor;
        }
        if (_ghscConfig.hasOwnProperty("duration")) {
            duration=_ghscConfig.duration;
        }
        ...
        $("#ghscAnalysisPanel").append(renderTpl(baseTpl, option));
        ...
        $optContainer.on('click', 'a', function () {
            ...
            switch (opt) {
                case 'draw':
                    if(_ghscConfig.custom===true){
                        var reSymbol=EsriSymbolsCreator.createSimpleFillSymbol(EsriSymbolsCreator.fillStyle.STYLE_SOLID,
                            ...
                            new Color(fillColor));
                    ghscAnalysis.draw('polygon', true,reSymbol).then(function (obj) {
                        //将对象渲染到页面列表中
                        if (obj != undefined) {
                            _appendList(obj);
                            $clearBtn.show();
                ...
                case 'analysis':
                    ...
                    try {
                        if (_ghscConfig.level=="bottom") {
                            showResultPanel();
                            ghscAnalysis.setTargetName("ghResultIframe");
                            ghscAnalysis.setLevel(_ghscConfig.level);
                        }
        ...
        $('#analysis-file-input-' + ghscAnalysis.getId()).on('change', function () {
            if(_ghscConfig.custom===true){
                var reSymbol=EsriSymbolsCreator.createSimpleFillSymbol(EsriSymbolsCreator.fillStyle.STYLE_SOLID,
                    EsriSymbolsCreator.createSimpleLineSymbol(EsriSymbolsCreator.lineStyle.STYLE_SOLID, new Color(lineColor), 2),
                    new Color(fillColor));
                ghscAnalysis.importFile($('#analysis-file-input-' + ghscAnalysis.getId()),reSymbol);
            }
            else {
    ...
    function showResultPanel() {
        ...
        // 绑定数据
        $("#result-container .content").empty();
        $("#result-container .content").append(renderTpl($("#ghRresultPanelTpl").html(), {}));

        var oFrm = $("#ghResultIframe")[0];
        var count=0;
        // var explorer = navigator.userAgent;
        // if (!!window.ActiveXObject || "ActiveXObject" in window) {
        //     oFrm.onreadystatechange = function (){
        //         if(this.readyState==='complete'){
        //             if(count!==2){count=count+1;return;}else{
        //                 $("#ghIframeLoading").remove();
        //                 count=0;
        //             }
        //         }else {return;}
        //     }
        ...
        oFrm.onload = oFrm.onreadystatechange = function () {
            if (!!window.ActiveXObject || "ActiveXObject" in window) {//判断是否是ie
                if(this.readyState==='complete'){//ie8下有readyState
                    if(count!==1){count=count+1}else{
                    $("#ghIframeLoading").remove();}
                }else if(this.readyState===undefined){//ie11下无readyState
                    if(count!==1){count=count+1}else{
                    $("#ghIframeLoading").remove();}
                }
            }
            else if(this.readyState===undefined){//非ie
                $("#ghIframeLoading").remove();
            }
        };
            ...
            window.doBottomLoc = function (fc) {
                ...
                if (obj.SHAPE !== undefined) {
                    ...
                    if (typeof obj.SHAPE === 'string') {
                        obj.SHAPE = $.parseJSON(obj.SHAPE);
                    }
                    var featureObj = JsonConverters.toEsri(obj.SHAPE, _map.spatialReference);
                    var gras = [];
                    if (featureObj.hasOwnProperty("features") && lang.isArray(featureObj.features)) {
                        arrayUtil.forEach(featureObj.features, function (feature) {
                    ...
                    var pos={};
                    var detail=obj.SHAPE.coordinates[0];
                    var m=0,n=0;
                    $.each(detail[0],function (index,item) {
                        m+=item[0];
                        n+=item[1];
                    });
                    var temp = {};
                    temp.wkid = _map.spatialReference.wkid;
                    pos.spatialReference = temp;
                    pos.x=m/detail[0].length;
                    pos.y=n/detail[0].length;
                    var content=
                        "<div><label>类型名称:</label><label>"+obj.DLMC+"</label></div>"+
                        "<div><label>类型代码:</label><label>"+obj.DLBM+"</label></div>"+
                        "<div><label>地块面积:</label><label>"+obj.OG_SHAPE_AREA+"</label></div>"+
                        "<div><label>标识码:</label><label>"+obj.BSM+"</label></div>"+
                        "<div><label>面积:</label><label>"+obj.SHAPE_AREA+"</label></div>";
                    mapPopup.setContent(content);
                    mapPopup.setTitle(obj.DLMC);
                    mapPopup.openPopup(pos);
                    // mapInfoWindow.showDetail(information, pos).then(
                    //     function () {
                    //         $("#areaName").html(obj.DLMC);
                    //         $("#areaNum").html(obj.DLBM);
                    //         $("#area").html(obj.OG_SHAPE_AREA);
                    //         $("#areaBSM").html(obj.BSM);
                    //         $("#shapeArea").html(obj.SHAPE_AREA);
                    //     }
                    // )
                }
            }
                ...
                if (fcs !== undefined) {
                    ...
                    arrayUtil.forEach(fcs,function (fc) {
                        fc.features.forEach(function (feature) {
                            ...
                            var highlightFeatureOpacit = 0.1;
                            if (featureObj.hasOwnProperty('rings')) {
                                var g = new Graphic({geometry: featureObj});
                                gras.push(g);
                                // 根据type="gh"决定颜色
                                MapUtils.highlightFeaturesWithStyle(gras, null, highlightFeatureOpacit,null,"gh");
                                _map.graphics.add(g);
                            }
                        })
                    })

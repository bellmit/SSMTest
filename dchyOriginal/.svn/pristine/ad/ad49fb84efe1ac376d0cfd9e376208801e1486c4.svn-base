...
export default {
    ...
    beforeRouteLeave (to, from, next) {
        if(to.fullPath.startsWith("/changzhou/htbadj/add")&&to.query.type!="add"){
    ...
    methods: {
        ...
        // 查看操作
        viewDetail(data){
            this.$router.push({ path: '/changzhou/htbadj/add', query: {type: "view", chxmid:data.CHXMID,slbh: data.SLBH} });
        },
        // 编辑操作
        edit(data){
            this.$router.push({ path: '/changzhou/htbadj/add', query: {type: "edit",chxmid:data.CHXMID,slbh: data.SLBH} });
        },
        // 新增登记单
        addDjd(){
            initDjdData().then(res => {
                this.$router.push({path: '/changzhou/htbadj/add', query: {slbh: res.data.slbh,xqfbbh:res.data.xqfbbh,chxmid:res.data.chxmid,slsj: res.data.slsj, type: "add"}})
            })
        },
        deleteOpr(){
        ...
        // 备案
        recordData(data){
            this.$router.push({ path: '/changzhou/htbadj/add', query: {type: "record",chxmid:data.CHXMID,slbh: data.SLBH, xmly: data.XMLY} });
        },
        // 暂停恢复终止
        stopAndRestore(){

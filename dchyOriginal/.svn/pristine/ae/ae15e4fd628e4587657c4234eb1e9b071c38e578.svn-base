...
<div>
    <ul class="nav nav-tabs">
        ...
        <li><a href="#tab_xz_4" data-toggle="tab">地块定位</a></li>
    </ul>
</div>
<div class="tab-content" style="overflow: auto;">
    <div id="tab_xz_4" class="tab-pane fade" style="overflow: auto">
        <button class="btn btn-default btn-loc" data-geo='${tplData.nydGeo!}'>压盖农用地地块</button>
        <button class="btn btn-default btn-loc" data-geo='${tplData.jsydGeo!}'>压盖建设用地地块</button>
        <button class="btn btn-default btn-loc" data-geo='${tplData.wlydGeo!}'>压盖未利用地地块</button>
    </div>
    <div id="tab1" class="tab-pane fade in active" style="overflow: auto">
        <#if (tplData.result?size>0)&&(tplData.result[0].categoryA??)>
            <table>
                <tr>
                    <th><#if tplData.level??&&tplData.level=="mas">地块名称<#else >行政区</#if></th>
                    <th>合计</th>
                    <#assign info=tplData.result[0].categoryA>
                    <#list info as infoItem>
                       <#if infoItem["dlbm"]?contains(",")><#else ><th <#if infoItem["dlbm"]=='12'>class="mediumWidth"</#if>>${infoItem["dlmc"]!}<#if infoItem["dlbm"]?contains(',')><#else >
    ...
    <div id="tab2" class="tab-pane fade" style="overflow: auto;">
        <#if (tplData.result?size>0)&&(tplData.result[0].categoryB??)>
            ...
            <table>
                <tr>
                    <th rowspan="2"><#if tplData.level??&&tplData.level=="mas">地块名称<#else >行政区</#if></th>
                    <th rowspan="2">合计</th>

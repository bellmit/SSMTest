<template>
    <div class="review-check">
        <div class="form-title">
            ...
            <div>
                <Button type="primary" class="btn-h-34 bdc-major-btn" v-if="isVerify" @click="clickHandler">审核</Button>
                <Button type="primary" class="btn-h-34 bdc-major-btn" v-if="isMake" @click="dataUpload">数据上传</Button>
                <Button type="primary" class="btn-h-34 bdc-major-btn" v-if="isDataApply" @click="resultApply">数据交付</Button>
         ...
         <Form class="form-edit" @on-validate="validateChecked" ref="commission-form" :model="applyInfo" :rules="ruleInline" :label-width="114">
            <Row>
                 <i-col span="12">
                    <FormItem v-model="applyInfo.babh" prop="babh" label="项目编号">
                        <Input :readonly="readonly" v-model="applyInfo.babh"/>
                    </FormItem>
                </i-col>
                <i-col span="12">
                    <FormItem v-model="applyInfo.gcbh" prop="gcbh" label="项目代码">
...
export default {
    ...
    mounted() {
        ...
        this.processDefName = this.$route.query.processDefName || util.getParentSearchParams("processDefName") || "";
                ...
                if(valid){
                    ...
                    applicationInfoView(this.verifyForm).then(res => {
                        ...
                        if(this.verifyForm.shjg === "0"){
                            window.parent.sendFilters = "办结"
                        } else {
                            window.parent.sendFilters = "制作"
                        }
                    })
                }
            })

...
<#assign gdzldbfxmas = result.gdzldbfxmas!/>
...
<#assign gdzldbfxmasexport = gdzldbfxmas.excelData!/>
...
<#macro generateSummaryText fid value number fixed>
    <#if value.total &gt;0||number&gt;0>
        <#switch fid>
            <#case "xz">
                本地块与${value.alias!}图层叠加，压盖农用地<span class="number">${(value.nyd?number)?string(fixed)}</span>m<sup>2</sup>,
                ...
                <#break/>
            <#case "gh">
                本地块与${value.alias!}图层叠加分析,规划土地用途区图层面积<span class="number">${(value.tdytq?number)?string(fixed)}</span>m<sup>2</sup>,其中,
...
<@aBase.tpl showHeader="false" css=cssContent bootVersion="-v3">
                    ...
                    <div id="Summary" class="tab-pane fade in">
                        <div class="well well-lg summary">
                            <#if result??>
                                <ol>
                                    <#if ogArea??>
                                        <li>本地块面积 <span class="number">${(Pass!)?string(fixed)}</span>m<sup>2</sup></li>
                                    </#if>
                                    <#list result?keys as key>
    ...
    <script type="text/javascript">
        ...
        function exportExcel() {
            ...
            switch (activeId) {
                ...
                case 'gyjsydfx':
                    ...
                    break;
                case 'gdzldbfxmas':
                    var gdzlData="${gdzldbfxmasexport}";
                    openPostWindowXZ("<@com.rootPath/>/geometryService/export/analysis", gdzlData, "gdzldbfx_mas.xml");
                    break;
                default: {
        ...
        function gotoLocation(geo,type) {

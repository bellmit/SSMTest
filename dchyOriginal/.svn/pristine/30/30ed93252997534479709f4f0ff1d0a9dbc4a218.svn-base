...
define(["dojo/_base/declare",
    ...
    var _map,cxghAnalysis,_label,_analysisConfig,_id;
    var subscribe;
    ...
    function _init() {
        ...
        cxghAnalysis = new BaseAnalysis(BaseAnalysis.TYPE_BB_CXGH, _map);
        cxghAnalysis.setAppConfig(appConfig);
        cxghAnalysis.setDataSource(_analysisConfig.dataSource);
        cxghAnalysis.setAnalysisLyr(_analysisConfig.layerName);
        cxghAnalysis.setOutFields(_analysisConfig.returnFields);
        cxghAnalysis.setTitleField(_analysisConfig.titleField);
        cxghAnalysis.setFieldAlias(_getFieldAlias());
        cxghAnalysis.setScopeLayers(_analysisConfig.scopeLayers);
        if (_analysisConfig.hasOwnProperty("link")) {
            var _link = _analysisConfig.link;
            if (_link.hasOwnProperty("url") && _link.url != "") {
                cxghAnalysis.setFreeParams({iframeUrl: _link.url});
            }
        }
        if(_analysisConfig.responseType){
            cxghAnalysis.setResponseType(_analysisConfig.responseType);
        ...
        if(_analysisConfig.hasOwnProperty("yx")){
            cxghAnalysis.setYx(_analysisConfig.yx);
        }
        //根据配置控制页面显示内容
        var option = {
            listId: "commonList".concat(cxghAnalysis.getId()),
            id: cxghAnalysis.getId(),
            queryModeOn: lang.isArray(_analysisConfig.scopeLayers)
        };
        lang.mixin(option,_analysisConfig);
        $("#cxghAnalysisTitle").append(_label);
        $("#cxghAnalysisPanel").append(renderTpl(baseTpl, option));
        $optContainer = $('#' + cxghAnalysis.getId());
        $selectContainer = $('#select_' + cxghAnalysis.getId());
        $listContainer = $("#commonList".concat(cxghAnalysis.getId()));
    ...
    function _addListener(){
        $optContainer.on('click', 'a', function () {
            ...
            switch (o) {
                case 'draw':
                    cxghAnalysis.draw('polygon', true).then(function (obj) {
                        //将对象渲染到页面列表中
                        if (obj != undefined) {
                            _appendList(obj);
                        }
                ...
                case 'imp':
                    ...
                    break;
                case 'analysis':
                    cxghAnalysis.setAnalysisGeometry();
                    try {
                        cxghAnalysis.doAnalysis();
                    } catch (e) {
                        alert(e.message);
                    }
                    break;
                case 'exp':
                    cxghAnalysis.exportFeature(null);
                    break;
                case 'clear':
                    clearHandle();
                    break;
        ...
        $selectContainer.on('click', 'a', function () {
            ...
            cxghAnalysis.draw(drawType,false).then(function(r){
                cxghAnalysis.queryGeo(r);
            });
        });
        //导入事件监听
        listenerFileInput();
    ...
    function clearHandle() {
        cxghAnalysis.clear().then(lang.hitch(this,function(){
            $clearBtn.hide();
            $listContainer.empty();
        }));
        _map.graphics.clear();
        var fileInput = document.getElementById('analysis-file-input-' + cxghAnalysis.getId());
        //清除fileInput内容
    ...
    function listenerFileInput(){
        $('#analysis-file-input-' + cxghAnalysis.getId()).off('change');
        $('#analysis-file-input-' + cxghAnalysis.getId()).on('change',function(){
            cxghAnalysis.importFile( $('#analysis-file-input-' + cxghAnalysis.getId()));
        });
        setTimeout(function(){
            listenerFileInput();
        },2000);
    ...
    function _appendList(obj){
        ...
        $listContainer.find(".a-r-btn").on('click', function () {
            ...
            var id=$(this).data("id");
            cxghAnalysis.findGraById(id).then(function(g){
                switch (t) {
                    case 'location':
                    ...
                    case 'analysis':
                        ...
                        cxghAnalysis.setAnalysisGeometry(JSON.stringify(geometry));
                        try {
                            cxghAnalysis.doAnalysis();
                        } catch (e) {
                            alert(e.message);

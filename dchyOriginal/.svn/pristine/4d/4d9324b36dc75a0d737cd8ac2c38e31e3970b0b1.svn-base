    ...
    <div>
        ...
        <Form class="form-edit" @on-validate="validateChecked" ref="commission-form" :model="commissionForm" :rules="ruleInline" :label-width="128">
                    ...
                    <div class="stage-tree">
                        <div class="select-row-tree">
                            <div v-for="(tree,index) in treeList" :key="index">
                                <Tree @on-check-change="checkChange" ref="tree" :data="tree" show-checkbox></Tree>
                        </div>
                        </div>
                    </div>
                </FormItem>
            ...
            <Row>
                ...
                <i-col span="8">
                    <FormItem v-model="commissionForm.gcbh" prop="gcbh" class="required-link" label="项目代码 ">
                        <Input :readonly="readonly" placeholder="填写示例: 2019-320412-78-01-570112" v-model="commissionForm.gcbh"/>
                    </FormItem>
                </i-col>
                <i-col span="8">
        ...
        <uploadHtFile
            ...
            :glsxid="chxmid"
            :readonly="true"
            :url="`/fileoperation/getsjclforht`"
            :needclsx="true"
            :clsxList="clsxList"
            :ssmkid="'22'"
        ></uploadHtFile>
        ...
        >
            ...
            <Form v-if="visible" class="form-edit" :hide-required-mark="hideRequiredMark" ref="verify-form" @on-validate="validateChecked" :model="verifyForm" :rules="fileRule" :label-width="114" >
                ...
                <FormItem v-model="verifyForm.fwxz" v-if="verifyForm.wtzt=='3'" prop="fwxz" label="房屋性质 ">
                    <RadioGroup v-model="verifyForm.fwxz">
                        <Radio label="0">商品房</Radio>
                        <Radio label="1">单位房</Radio>
                    </RadioGroup>
                </FormItem>
            ...
            <div class="form-title" v-if="verifyForm.wtzt=='3'">
                <div class="list-title">测绘体量</div>
            </div>
            <div v-if="verifyForm.wtzt=='3'" class="table-select">
                <Table
                    ref="chtlTable"
                    :showPage="false"
                    :showDefaultTool="true"
...
import { getDictInfo, getClsxDictInfo } from "../../../service/mlk"
...
export default {
    ...
    mounted() {
        ...
        if(this.$root.$data.getCommissionInfo()){
            ...
            this.checkedClsxList = typeof this.commissionForm.clsx == "object" ? this.commissionForm.clsx.map(c => c.clsxdm) : this.commissionForm.clsx ? this.commissionForm.clsx.split(",") : []
    ...
    methods: {
        ...
        // 生成测绘体量
		initChtlData(){
			let clsxList = this.clsxDictionaryInfo;
			let chtlData = [];
			this.checkedClsxList.forEach(clsx => {
        ...
        submit(){
            ...
            this.commissionForm.clsx = this.getSelectedClsx();
            let uploadList = [];
        ...
        getDictInfo(){
            ...
            getClsxDictInfo({zdlx: ["CLSX"],chxmid: this.chxmid}).then(res => {
                this.clsxDictionaryInfo = res.data.dataList || [];
                this.clsxList = res.data.dataList || [];
                this.renderTreeList();
            })
            getDictInfo(params).then(res => {
                ...
                if(this.showHtFj){
                    this.$refs["upload-file-ht"].getUploadList();
                }
            })
        },
        // 选择省
        ...
        verify(){
            ...
            this.$refs["verify-form"].validate(valid => {
                ...
                if(valid){
                    ...
                    this.verifyForm.chtl = this.chtlData

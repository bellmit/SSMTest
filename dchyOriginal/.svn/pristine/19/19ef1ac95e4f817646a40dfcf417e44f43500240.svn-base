...
define(["dojo/_base/declare",
    ...
    var _fileTimer, _msgHandler, _dltbClipResult, _xzdwClipResult;
    ...
    function _doClear() {
        ...
        layer.closeAll();
        _dltbClipResult = undefined;
        _xzdwClipResult = undefined;
    ...
    var xzdwSoeResult, dltbSoeResult;

    var fileUploadId = 'shpFileInput';
    ...
    function _addItem(file, tags, type) {
        ...
        console.time('analysis time used');
        $.ajaxFileUpload({
            fileElementId: fileUploadId,
            url: url,
            type: 'post',
            ...
            success: function (ret) {
                ...
                if (r.success) {
                    ...
                    console.log("------要素已成功添加至 portal ------", r.id);
                } else {
                    var err = r.error;
                    if (err.code === 409) {
                    ...
                    console.warn('-----添加要素至 portal 异常----', err);
                }
            }
        });
    }
    ...
    function _publishItem(itemId, name, type) {
        ...
        _ajaxRequest(url, data, true, false, function (ret) {
            ...
            if (ret.error) {
                ...
                console.warn('------发布要素异常----', ret.error.message);
            } else {
                //待分析图形服务地址
     ...
     * 进行分析
     * 分析分步骤：
     * - 分别 与 dltb 以及 xdw 进行 join
     * - 查询 join 结果 features
     * - 分别进行 soe 的计算
     * - 将上一步的结果 作为 参数 （需要是 在线模式） 传递到 jsgt-ga.jar 接口
     * - 显示分析结果
     * @private
     */
    function _doAnalysis() {
        // clear vars 
        dltbSoeResult = undefined;
        xzdwSoeResult = undefined;
            ...
            _join(AgsAPI.dltb, _shpFeatureLayer, 'dltb_join', function (data) {
                ...
                var clock = window.setInterval(function () {
                    _pollGpState('join', jobId, clock, function (ret) {
                        console.log('---[dltb-join] 结果---', ret.value.url);
                        _addMsg('[dltb-join] 结果:' + ret.value.url);
                        var tUrl = ret.value.url;
                        if (esriLang.isDefined(tUrl)) {
                            // 修改 maxRecordCount 参数
                            _updateDefinition(tUrl);
                            var clock = window.setInterval(function () {
                                _pollQueryUrl(tUrl, {outFields: 'TBBH,DLBM,DLMC,ZLDWDM,FQXZQDM'}, clock, function (ret) {
                                    dltbJoinResult = ret;
                                    console.log("----成功获取到dltb join 结果----");
                                    _callSoeDltb();
                                });
                            }, POLL_INTERVAL_TIME);
                        }
            ...
            _join(AgsAPI.xzdw, _shpFeatureLayer, 'xzdw_join', function (data) {
                ...
                var clock = window.setInterval(function () {
                    _pollGpState('join', jobId, clock, function (ret) {
                        ...
                        console.log('---[xzdw-join] 结果---', retUrl);
                        _addMsg('[xzdw-join] 结果:' + retUrl);
                        if (retUrl != undefined) {
                            // 修改 maxRecordCount 参数
                            _updateDefinition(retUrl);
                            var clock = window.setInterval(function () {
                                _pollQueryUrl(retUrl, {outFields: 'DLBM,DLMC,KD,KCTBBH1,KCTBDWDM1,KCTBBH2,KCTBDWDM2,FQXZQDM'}, clock, function (ret) {
                                    window.clearInterval(clock);
                                    xzdwJoinResult = ret;
                                    console.log("----成功获取到xzdw join 结果----");
                                    _callSoeXzdw();
                                });
                            }, POLL_INTERVAL_TIME);
                        }
     ...
     * @private
     */
    function _updateDefinition(featureService) {
        var url = lang.trim(featureService);
        url = url.substring(0, url.length - 2);
        if (url.indexOf('admin') < 0) {
            var arr = url.split('rest/');
            url = arr[0] + 'rest/admin/' + arr[1] + '/updateDefinition';
        }
        var data = {updateDefinition: dojoJson.stringify({maxRecordCount: 200000})};
        lang.mixin(data, {
            token: _token,
            f: 'json'
        });
        _ajaxRequest(url, data, false, true, function (ret) {
            var data = dojoJson.parse(ret);
            if (data.success === true) {
                console.log('修改参数 maxRecordCount 成功!');
            } else {
                console.warn('---warning---', '修改参数 maxRecordCount 失败!');
            }
        }, function (e) {
            console.error(e);
        }, true);
            ...
            success: function (data) {
                ...
                switch (jobStatus) {
                    ...
                    case GP_JOB_STATUS.Executing:
                        console.log('-----gp任务类型:[ ' + type + ' ] 任务ID:[' + jobId + '] 当前状态 [ ' + jobStatus + '] ------');
                    ...
                    case GP_JOB_STATUS.Failed:
                        // layer.closeAll();
                        console.warn('-----gp任务类型:[ ' + type + ' ] 任务ID:[' + jobId + '] 当前状态 [ ' + jobStatus + '] ------');
     ...
     * @private
     */
    function _callSoeXzdw() {
        if (esriLang.isDefined(shpFeature)) {
            if (esriLang.isDefined(xzdwJoinResult) && !esriLang.isDefined(xzdwSoeResult)) {
                _ajaxRequest(AgsAPI.soeUrl(), {
                    srcPath: '/arcgis/intersect',
                    featureclass1: dojoJson.stringify(shpFeature),
                    featureclass2: dojoJson.stringify(xzdwJoinResult),
                    targetPath: AgsAPI.soeTargetPath(),
                    token: _token,
                    f: 'json'
                }, false, false, function (ret) {
                    if (ret.hasOwnProperty('error')) {
                        console.warn('---soe error---', ret.error);
                        return false;
                    }
                    console.log('----xzdw soe success----', ret);
                    xzdwSoeResult = ret.resultfile;
                    _executeXzAnalysis();
                });
            }
        } else
     ...
     * @private
     */
    function _callSoeDltb() {
        if (esriLang.isDefined(shpFeature)) {
            if (esriLang.isDefined(dltbJoinResult) && !esriLang.isDefined(dltbSoeResult)) {
                _ajaxRequest(AgsAPI.soeUrl(), {
                    srcPath: '/arcgis/intersect',
                    featureclass1: dojoJson.stringify(shpFeature),
                    featureclass2: dojoJson.stringify(dltbJoinResult),
                    targetPath: AgsAPI.soeTargetPath(),
                    token: _token,
                    f: 'json'
                }, false, false, function (ret) {
                    ...
                    console.log('----dltb soe success----', ret);
                    dltbSoeResult = ret.resultfile;
                    _executeXzAnalysis();
                });
            }
    ...
    function _executeXzAnalysis() {
        if (dltbSoeResult != undefined && dltbSoeResult != "") {
            var restUrl = AgsAPI.xzAnaServer();
            var data = {
                result1: AgsAPI.soeTomcatUrl().concat(dltbSoeResult),
                f: 'json'
            };
            if (xzdwSoeResult != undefined) {
                lang.mixin(data, {
                    result2: AgsAPI.soeTomcatUrl().concat(xzdwSoeResult)
                });
            }
            $.ajax({
                type: "POST",
                ...
                data: data,
                success: function (data) {
                    console.timeEnd('analysis time used');
                    $("#msgContainer").empty();
                    layer.close(_msgHandler);
                    console.log(data);
                    if (typeof data === 'string') {
                        data = data.replace('success_jsonpCallback', '').replace(/\(/g, '').replace(/\);/g, '');
                        _showResult(dojoJson.parse(data));
     ...
     * @private
     */
    function _ajaxRequest(url, data, loadingState, sync, callback, failBack, reqProxy) {
        var tmp;
        $.ajax({
            url: reqProxy === true ? proxyUrl + "?dataType=json&requestUrl=" + url : url,
            method: 'post',
            data: data,
            timeout: 360000,
            dataType: 'json',
     ...
     * @private
     */
    function _overlay(inputLayer, clipLayer, outputName, callback) {
        var data = {
        ...
        _ajaxRequest(AgsAPI.spGpServer() + GP_TASKS.OverlayLayers + "/submitJob", data, true, false, function (ret) {
            var clock = window.setInterval(function () {
                _pollGpState('overlay', ret.jobId, clock, lang.hitch(this, callback));
            }, POLL_INTERVAL_TIME);
        });
    }

<template>
	<div>
		...
		<div v-if="current == 1">
			...
			<uploadFileInfo :glsxid="mlkid" :ssmkid="'10'"></uploadFileInfo>
		</div>
		<div v-if="current == 2">
			<reviewInfo ref="review-info" :reviewData="reviewData" @shjgchange="shjgChange"></reviewInfo>
		</div>
		<div class="submit-back">
			<Button v-if="current == 1" type="primary" class="width-100 btn-h-36 bdc-major-btn" @click="nextStep()">下一步</Button>
			<Button v-if="current == 2" type="primary" class="width-100 btn-h-36 btn-cancel" @click="preStep()">上一步</Button>
			<Button v-if="current == 2" type="primary" class="width-100 btn-h-36 bdc-major-btn margin-left-10" @click="save()">确认办结</Button>
		</div>
	</div>
</template>
<script>

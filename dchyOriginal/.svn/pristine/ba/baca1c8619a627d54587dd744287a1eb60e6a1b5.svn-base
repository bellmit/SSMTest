...
define(["esri/graphic",
    ...
    function (Graphic, declare, lang, arrayUtil, topic, dojoJSON, RandomUuid, BaseWidget, BaseAnalysis, JsonConverters, GeoDataStore, Handlebars,
              esriLang, Color, EsriSymbolsCreator, MapUtils, layer, GeometryIO, FeatureSet, BasicTpl, ListItem) {
        ...
        var baseReport, exportSelTpl, exportBtn;
        ...
        function _init() {
            ...
            //初始化对象
            xzAnalysis = new BaseAnalysis(BaseAnalysis.TYPE_TDLYXZ, _map);
            xzAnalysis.setAppConfig(appConfig);
            xzAnalysis.setDltbLyr(_xzConfig.dltb);
            xzAnalysis.setLocQuery(_xzConfig.locQuery);
            xzAnalysis.setXzdwLyr(_xzConfig.xzdw);
            xzAnalysis.setLxdwLyr(_xzConfig.lxdw);
            xzAnalysis.setMethodType(_xzConfig.methodType);
            xzAnalysis.setDecimal(_xzConfig.decimal);
            xzAnalysis.setDataSource(_xzConfig.dataSource);
            xzAnalysis.setAreaUnit(_xzConfig.areaUnit || BaseAnalysis.UNIT_SQUARE_METERS);
            xzAnalysis.setScopeLayers(_xzConfig.scopeLayers);
            xzAnalysis.setTdlyxzfxBz(_xzConfig.tdlyxzfxBz);
            xzAnalysis.setBjZipImportUnit(_xzConfig.bjZipImportUnit);
            xzAnalysis.setReportInfoUrl(_xzConfig.reportInfoUrl);
            xzAnalysis.setGroupBy(_xzConfig.groupBy);
            if (_xzConfig.showXzqmc != undefined && _xzConfig.showXzqmc != null) {
                xzAnalysis.setShowXzqmc(_xzConfig.showXzqmc);
            }
            if (_xzConfig.showDetail != undefined && _xzConfig.showDetail != null) {
                xzAnalysis.setShowDetail(_xzConfig.showDetail);
            }
            if (_xzConfig.hasOwnProperty("yx")) {
                xzAnalysis.setYx(_xzConfig.yx);
            }
            if (_xzConfig.hasOwnProperty("linkUrl")) {
                xzAnalysis.setLinkUrl(_xzConfig.linkUrl);
            }
            if (_xzConfig.hasOwnProperty("exportAnalysis")) {
                xzAnalysis.setExportResult(_xzConfig.exportAnalysis);
            }
            if (_xzConfig.hasOwnProperty("requestUrl")) {
                //如果配置中存在requestUrl则请求相应的地址进行分析，用于南通等地定制的土地利用现状分析
                xzAnalysis.setRequestUrl(_xzConfig.requestUrl);
            }
            if (_xzConfig.hasOwnProperty("fillColor")) {
                fillColor = _xzConfig.fillColor;
            }
            if (_xzConfig.hasOwnProperty("locFillColor")) {
                locFillColor = _xzConfig.locFillColor;
            }
            if (_xzConfig.hasOwnProperty("lineColor")) {
                lineColor = _xzConfig.lineColor;
            }
            if (_xzConfig.hasOwnProperty("duration")) {
                duration = _xzConfig.duration;
            }
            if (_xzConfig.hasOwnProperty("exportBtn")){
                exportBtn= _xzConfig.exportBtn;
            }
            //初始化变量
            $xzAnalysisPanel = $("#xzAnalysisPanel");
            //根据配置控制页面显示内容
            var option = {
                listId: "xzList",
                id: xzAnalysis.getId(),
            ...
            if (lang.isArray(_xzConfig.year) && _xzConfig.year.length > 0) {
                ...
                //默认选中最新的年份数据
                $xzAnalysisPanel.append(renderTpl($("#xzYearSelectTpl").html(), {years: _xzConfig.year}));
                //多选
            ...
            if (xzAnalysis.locQuery == true) {
                ...
                $("#dataTypeUl>li").click(function () {
                    var selva = $(this).text();
                    var value = $(this).attr("code")
                    $(this).parents("#dataTypeUl").siblings(".div-dataSource-top").find("span").text(selva);
                    $(this).parents("#dataTypeUl").siblings(".div-dataSource-top").find("span").attr("code", value);
                    $(this).parent("ul").hide();
                ...
                geoDataStore.fetch(GeoDataStore.SK_IDENTIFY).then(lang.hitch(this, onShareData));
        ...
        function onShareData(data) {
            if (data !== undefined) {
                ...
                switch (type) {
                    ...
                    case GeoDataStore.SK_IDENTIFY: {
                        if (esriLang.isDefined(data))
                            baseReport = data.features;
                        break;
                    }
                }
                    ...
                    case 'draw':
                        if (_xzConfig.custom === true) {
                            var reSymbol = EsriSymbolsCreator.createSimpleFillSymbol(EsriSymbolsCreator.fillStyle.STYLE_SOLID,
                                ...
                                new Color(fillColor));
                            xzAnalysis.draw('polygon', true, reSymbol).then(function (obj) {
                                //将对象渲染到页面列表中
                                if (obj != undefined) {
                                    _appendList(obj);
                                    $clearBtn.show();
            ...
            $('#analysis-file-input-' + xzAnalysis.getId()).on('change', function () {
                if (_xzConfig.custom === true) {
                    var reSymbol = EsriSymbolsCreator.createSimpleFillSymbol(EsriSymbolsCreator.fillStyle.STYLE_SOLID,
                        EsriSymbolsCreator.createSimpleLineSymbol(EsriSymbolsCreator.lineStyle.STYLE_SOLID, new Color(lineColor), 2),
                        new Color(fillColor));
                    xzAnalysis.importFile($('#analysis-file-input-' + xzAnalysis.getId()), reSymbol);
                } else {

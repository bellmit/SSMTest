...
; !
    function(a, b) {
            ...
            f = {
                ...
                config: function(a, b) {
                    ...
                    return a = a || {},
                        ...
                        f.use(f.path.indexOf("thirdparty/layer/")>-1?"skin/layer-omp.css":"../thirdparty/layer/skin/layer-omp.css", a.extend && a.extend.length > 0 ?
                            function g() {
                                var c = a.extend;
        ...
        g.pt = g.prototype;
        var h = ["layui-layer", ".layui-layer-title", ".layui-layer-main", ".layui-layer-dialog", "layui-layer-iframe", "layui-layr-content", "layui-layer-btn", "layui-layer-close","layui-layer-optbtn"];
        h.anim = ["layui-anim", "layui-anim-01", "layui-anim-02", "layui-anim-03", "layui-anim-04", "layui-anim-05", "layui-anim-06"],
            ...
            g.pt.vessel = function(a, b) {
                ...
                return f.zIndex = g,
                    b([f.shade ? '<div class="layui-layer-shade" id="layui-layer-shade' + d + '" times="' + d + '" style="' + ("z-index:" + (g - 1) + "; background-color:" + (f.shade[1] || "#000") + "; opacity:" + (f.shade[0] || f.shade) + "; filter:alpha(opacity=" + (100 * f.shade[0] || 100 * f.shade) + ");") + '"></div>': "", '<div class="' + h[0] + " " + (h.anim[f.shift] || "") + (" layui-layer-" + e.type[f.type]) + (0 != f.type && 2 != f.type || f.shade ? "": " layui-layer-border") + " " + (f.skin || "") + '" id="' + h[0] + d + '" type="' + e.type[f.type] + '" times="' + d + '" showtime="' + f.time + '" conType="' + (a ? "object": "string") + '" style="z-index: ' + g + "; width:" + f.area[0] + ";height:" + f.area[1] + (f.fix ? "": ";position:absolute;") + '">' + (a && 2 != f.type ? "": k) + '<div class="layui-layer-content' + (0 == f.type && -1 !== f.icon ? " layui-layer-padding": "") + (3 == f.type ? " layui-layer-loading" + f.icon: "") + '">' + (0 == f.type && -1 !== f.icon ? '<i class="layui-layer-ico layui-layer-ico' + f.icon + '"></i>': "") + (1 == f.type && a ? "": f.content || "") + '</div><span class="layui-layer-setwin">' +
                        ...
                        function(){
                            var a="";
                            "string" == typeof f.optbtn && (f.optbtn = [f.optbtn]);
                            for (var i = 0,
                                     l = f.optbtn.length; l > i; i++){
                                var ob=f.optbtn[i];
                                a += ob[0]?'<a class="btn btn-info btn-sm"'+ (ob[2]?' data-token="'+ ob[2]+'">':'>') +
                                (ob[1]?('<i class="' + ob[1] + '"></i>&nbsp;'):'')+ ob[0]+"</a>":"";
                            }
                            return '<div class="' + h[8] + '">' + a + "</div>"
                        }():"")+"</div>"], k),
                    c
            },
            g.pt.creat = function() {
            ...
            g.pt.callback = function() {
                ...
                b.openLayer(),
                    ...
                    d.find("." + h[8]).children("a").on("click", function(){
                        var e=c(this).index();
                        g["optbtn"+(e+1)]&&g["optbtn"+(e+1)](b.index,this);
                    }),
            ...
            e.run(),
                f.use(f.path.indexOf("thirdparty/layer/")>-1?"skin/layer-omp.css":"../thirdparty/layer/skin/layer-omp.css")
        } ()
    } (window);
...
define(["dojo/_base/declare",
    ...
    var _map,tddjAnalysis,_label,_analysisConfig,_id;
    var subscribe;
    ...
    function _init() {
        ...
        tddjAnalysis = new BaseAnalysis(BaseAnalysis.TYPE_TDDJ, _map);
        tddjAnalysis.setAppConfig(appConfig);
        tddjAnalysis.setDataSource(_analysisConfig.dataSource);
        tddjAnalysis.setAnalysisLyr(_analysisConfig.layerName);
        tddjAnalysis.setOutFields(_analysisConfig.returnFields);
        tddjAnalysis.setTitleField(_analysisConfig.titleField);
        tddjAnalysis.setFieldAlias(_getFieldAlias());
        tddjAnalysis.setScopeLayers(_analysisConfig.scopeLayers);
        if (_analysisConfig.hasOwnProperty("link")) {
            var _link = _analysisConfig.link;
            if (_link.hasOwnProperty("url") && _link.url != "") {
                tddjAnalysis.setFreeParams({iframeUrl: _link.url});
            }
        }
        if(_analysisConfig.responseType){
            tddjAnalysis.setResponseType(_analysisConfig.responseType);
        ...
        if(_analysisConfig.hasOwnProperty("yx")){
            tddjAnalysis.setYx(_analysisConfig.yx);
        }
        //根据配置控制页面显示内容
        var option = {
            listId: "commonList".concat(tddjAnalysis.getId()),
            id: tddjAnalysis.getId(),
            queryModeOn: lang.isArray(_analysisConfig.scopeLayers)
        };
        lang.mixin(option,_analysisConfig);
        $("#tddjAnalysisTitle").append(_label);
        $("#tddjAnalysisPanel").append(renderTpl(baseTpl, option));
        $optContainer = $('#' + tddjAnalysis.getId());
        $selectContainer = $('#select_' + tddjAnalysis.getId());
        $listContainer = $("#commonList".concat(tddjAnalysis.getId()));
    ...
    function _addListener(){
        $optContainer.on('click', 'a', function () {
            ...
            switch (o) {
                case 'draw':
                    tddjAnalysis.draw('polygon', true).then(function (obj) {
                        //将对象渲染到页面列表中
                        if (obj != undefined) {
                            _appendList(obj);
                        }
                ...
                case 'imp':
                    ...
                    break;
                case 'analysis':
                    tddjAnalysis.setAnalysisGeometry();
                    try {
                        tddjAnalysis.doAnalysis();
                    } catch (e) {
                        alert(e.message);
                    }
                    break;
                case 'exp':
                    tddjAnalysis.exportFeature(null);
                    break;
                case 'clear':
                    clearHandle();
                    break;
        ...
        $selectContainer.on('click', 'a', function () {
            ...
            tddjAnalysis.draw(drawType,false).then(function(r){
                tddjAnalysis.queryGeo(r);
            });
        });
        //导入事件监听
        listenerFileInput();
    ...
    function clearHandle() {
        tddjAnalysis.clear().then(lang.hitch(this,function(){
            $clearBtn.hide();
            $listContainer.empty();
        }));
        _map.graphics.clear();
        var fileInput = document.getElementById('analysis-file-input-' + tddjAnalysis.getId());
        //清除fileInput内容
    ...
    function listenerFileInput(){
        $('#analysis-file-input-' + tddjAnalysis.getId()).off('change');
        $('#analysis-file-input-' + tddjAnalysis.getId()).on('change',function(){
            tddjAnalysis.importFile( $('#analysis-file-input-' + tddjAnalysis.getId()));
        });
        setTimeout(function(){
            listenerFileInput();
        },2000);
    ...
    function _appendList(obj){
        ...
        $listContainer.find(".a-r-btn").on('click', function () {
            ...
            var id=$(this).data("id");
            tddjAnalysis.findGraById(id).then(function(g){
                switch (t) {
                    case 'location':
                    ...
                    case 'analysis':
                        ...
                        tddjAnalysis.setAnalysisGeometry(JSON.stringify(geometry));
                        try {
                            tddjAnalysis.doAnalysis();
                        } catch (e) {
                            alert(e.message);

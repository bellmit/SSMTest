...
define(["dojo/_base/declare",
    ...
    function (declare, arrayUtil, lang, topic, locale, parser, Draw, SpatialReference, GeometryService, BufferParameters, Point,
            ...
            if (typeof camera === "string") {
                ...
                if (tmp !== undefined) {
                    ...
                    if (platConfig && (platConfig.platType === 'hw' || platConfig.platType === "dh")) {
                        ...
                        openIvsVideo(tmp.name,tmp.indexCode,tmp.vcuId,platConfig,type,pNo,tmp.regionName);
                        return;
                    }
                    _a.devicelist = [
                        {
                ...
                if (lang.isArray(camera)) {
                    arrayUtil.forEach(camera, function (item) {
                        ...
                        if (platConfig && (platConfig.platType === 'hw' || platConfig.platType === "dh")) {
                            ...
                            openIvsVideo(item.name,item.indexCode,item.vcuId,platConfig,type,pNo,item.regionName);
                            return;
                        }
                        v.devicelist = [{
                    ...
                    if (platConfig && (platConfig.platType === 'hw' || platConfig.platType === "dh")) {
                        ...
                        openIvsVideo(camera.name,camera.indexCode,camera.vcuId,platConfig,type,pNo,camera.regionName);
                        return;
                    }
                    vcu.devicelist = [
                        {
         ...
         * @param pNo
         */
        function openIvsVideo(name,cameraId,vcuId,platform,type,pNo,regionName){
            var cmrvParam = {
            ...
            var viewStr = JSON.stringify(cmrvParam);
            ...
            vcu.devicelist = [
                {
                    DeviceName: encodeURIComponent(name),
                    IndexCode: cameraId
                }
            ];
            vcu.VcuName=regionName;
            vcuList.push(vcu);
            postVideoLog(platform, vcuList, 0);

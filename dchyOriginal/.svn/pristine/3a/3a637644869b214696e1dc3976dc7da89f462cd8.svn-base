...
define(["dojo/_base/declare",
    ...
    function (declare, arrayUtil, lang, topic, locale, parser, Draw, SpatialReference, GeometryService, BufferParameters, Point,
              ...
              MapInfoWindow, DateUtils, Environment, SerializeForm, ImagesViewerUtil, HbarsUtils,
        ...
        //var otherLoginName =otherLoginName||"";
        ...
        function receiveMessage(obj) {
            ...
            if(messageCall[data.departType]&&messageCall[data.departType][data.type]){
                debugger
                messageCall[data.departType][data.type](data);
            }
            dealWithMessage(obj);
        }
        var messageCall={};
         ...
         * @param obj
         */
        function sendMessage(departType,type,data,callBack) {
            debugger
            var message={};
            message.departType = departType;
            message.type=type;
            message.data = data;
            var content= JSON.stringify(message);
            if(callBack){
                messageCall[departType] = messageCall[departType]||{};
                messageCall[departType][type]  = callBack;
            }
            window.parent.postMessage(content,"*");
        }

        function dealWithMessage(data) {
            var data  =JSON.parse(data.data);
            if(data.departType=="proList"){
                switch (data.type){
                    case "refreshPage":
        ...
        function initComponent() {
            ...
            switch (partType) {
                ...
                case "proPane":
                    ...
                    break;
                case "cameraPane":
                    openCameraPane(indexCode);
                    break;
                case "imgViewer":
                    debugger
                    openProImgViewer(proId);
                    break;
                case "imgHistory":
                    debugger
                    openProImgHistory();
            ...
            //openProList();
            //openCameraPane("021200572640101");
            //openProPane("f167e31aab3a2c92825b67e0326e0006");
            //openProImgViewer("f167e31aab3a2c92825b67e0326e0006");
        }
        ...
        function fetchVideoData() {
            var url=root + "/video/fetch/all?loginName="+otherLoginName;
            //var url=root + "/video/fetch/all";
            $.ajax({
                url: url,
                success: function (r) {
                    if (r.length > 0) {
        ...
        function getOffset() {
            ...
            var top = parent.document.body.clientHeight / 2;
            return [top + 'px', left];
        }
         ...
         * @param gra
         */
        function cameraGraClickHandler(attr) {
                    ...
                    $.getJSON(root + "/project/list?indexCode=" + attr.indexCode, function (ret) {
                        if (lang.isArray(ret)) {
                            ...
                            if (pros.length === 0) {
                                ...
                                sendMessage("cameraPane","resizePop",{"height":height,"width":width});
                                return;
                            }
                        ...
                        sendMessage("cameraPane","resizePop",{"height":height,"width":600});
                    });
                },
                cancel: function () {
                    sendMessage("cameraPane","closePop",null);
                },
            });
        ...
        function addProjectListeners() {
            ...
            $("#addBtn").off("click").on('click', function () {
                $btn = $("#addBtn");
                $btn.html('新建中');
                sendMessage("proList","createPro",null,function (evt) {
                    debugger
                    if ($btn) {
            ...
            $('#orderPanel li i').unbind("click").on('click', function () {
                ...
                //var postData = {ownerId: otherLoginName, orderField: orderField, order: order};
                var postData = {ownerId: 1, orderField: orderField, order: order};
                $.ajax({
                    url: ajaxUrl,
                    data: postData,
                    async: false
            ...
            sendMessage("proList","getHeight",null,function (data) {
                debugger
                var proContentHeight;
                proContentHeight = data.proHeight;
        ...
        function refreshProPage(page, condition) {
            pageSize = parseInt((($("#proContent").height()-50) / 70), 10) + 1;
            loadHandler = layer.load(2, {offset: ['300px', '245px']});
            var ajaxUrl = "/omp/project/page?ieRefresh=" + Math.uuid(32);
            var postData = {page: page, size: pageSize, ownerId: otherLoginName};
            //var postData = {page: page, size: pageSize, ownerId: 1};
        ...
        function renderProList(condition) {
            if (esriLang.isDefined(proList) && proList.length > 0) {
                    ...
                    switch (op) {
                        case 'loc': {
                            ...
                            if (results.length > 0) {
                                ...
                                sendMessage("proList","locPro",tmp);
                                // var pnt = new Point(tmp.x, tmp.y, __map.spatialReference);
                            }
                            break;
                        }
                        case 'record': {
                            debugger
                            //向父页面申请打开departType,type,data,callBack
                            sendMessage("imgViewer","showImgViewer",{"proId":proid});
                        ...
                        case 'del': {
                            sendMessage("proList","delProject",{"proId":proid},function (evt) {
                                if(evt["delPro"]){
                                        $.ajax({
                                            ...
                                            success: function (rp) {
                                                if (rp.result == true) {
                                                    ...
                                                    sendMessage("proList","delSuccess");
                                                }
                                            }
                                        });
                                }
                ...
                $(".pro-info-header").on('click', function () {
                    ...
                    // }
                    sendMessage("proPane","showPane",{"proId":proid});
                });
            }
            else {
                $("#proContent").empty();
        ...
        function openProDetail(pro) {
                ...
                $.ajax({
                    ...
                    success: function (cameraIds) {
                        arrayUtil.forEach(cameraIds, function (cameraId) {
                            ...
                            // });
                            var camera = queryCameraByIndex(cameraId);
                            results.push(camera);
            ...
            sendMessage("proPane","resizePop",{"height":height,"width":550});
        }
        ...
        function addHistoryImgListener(rs, proId) {
            ...
            $(".history-img-content").unbind('click').on('click', 'img', function () {
                ...
                sendMessage("imgHistory","showImg",param);
            ...
            $(".history-close-btn").unbind('click').on('click', function () {
                layoutManager.hideRightPanel();
                $('.pro-info-content').removeClass('record-clicked');
                sendMessage("imgViewer","hideImgViewer",null);
            });
        ...
        window.openProImgHistory = function () {
            //像父页面请求参数
            sendMessage("imgHistory","getParam",null,function (data) {
                var param = data["param"];
                if(param.hasImgs) {
                    if (param.fileType != undefined && param.fileType === "mp4") {
                        //打开视频文件预览
                         open({
                            type: 2,
                            title: '视频播放',
                            shade: 0,
                            area: ['640px', '320px'],
                            content: ['/omp/video/mp4player?fileId=' + param.id, 'no']
                        });
                    } else {
                        var timestr = param.fileTimeStr;
                        var date = new Date(timestr);
                        ImagesViewerUtil.showImages(date, param.rs);
                        //添加上传照片刷新照片记录列表功能
                        var handle;
                        if (handle) handle.remove();
                        handle = topic.subscribe("img/upload/quarterRender", function (result) {
                            picRecords = result.datas;
                            quarterRender(result.year, result.season, result.proId, result.datas);
                            addHistoryImgListener(result.datas, param.proId);
                        });
                    }

                }else {
                    //没有任何数据时的上传图片按钮监听
                    ImagesViewerUtil.queryProImages(null, param.fileId);
                }
                var height = $(".layui-layer").height();
                var width = $(".layui-layer").width();
                window.sendMessage = sendMessage;
                sendMessage("imgHistory","resizePop",{"height":height,"width":width})
            });
        }

        window.openProImgViewer = function (proid,indexCode) {
            debugger
            if (!__config.multipleVideo) {
                renderHistoryRecords(proid, indexCode);
            } else {
        ...
        window.openProPane = function (proId) {
            debugger
            setTimeout(function(){
                $.ajax({
                    url:root+"/project/get/"+proId,
        ...
        window.openCameraPane =function (indexCode) {
            debugger
            var camera = queryCameraByIndex(indexCode);
            cameraGraClickHandler(camera);
        }
        
        window.openProList = function (ownerId) {
            $("#popupwin_area #panel-VideoPart").addClass("showImp");
        }
        return me;
    });
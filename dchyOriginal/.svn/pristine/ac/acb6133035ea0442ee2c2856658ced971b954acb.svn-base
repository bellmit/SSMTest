...
public interface ExchangeFeignService {
     ...
     */
    @Headers({"Content-Type: application/json", "Accept: application/json", "token: " + TokenCheckUtil.TOKEN})
    @RequestLine("POST /rest/v1.0/platform/gxchgcclxx/{babh}")
    Map getGxchgcclDownUrl(@RequestBody Map cgclmcList, @Param("babh") String babh);
    ...
    PfUserVo getUserByloginName(@Param("loginName") String loginName);
     ...
     */
    @Headers({"token: " + TokenCheckUtil.TOKEN})
    @RequestLine("POST /rest/v1.0/platform/department/list")
     ...
     */
    @Headers({"token: " + TokenCheckUtil.TOKEN})
    @RequestLine("GET /rest/v1.0/platform/role/list/organ/{organid}")
    List<PfRoleVo> getRoleListByOrganid(@Param("organid") String organid);

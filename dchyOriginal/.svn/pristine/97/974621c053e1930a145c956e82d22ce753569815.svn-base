<template>
  <div>
        ...
        <Row>
          <i-col span="6">
            <FormItem label="项目编号" class="form-list-search" prop="slbh">
              <Input type="text" class="form-search-item" @keydown.enter.native.prevent="getReviewList(1,formInline.size)" v-model="formInline.babh" placeholder="" />
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem label="项目代码" class="form-list-search" prop="gcbh">
              <Input type="text" class="form-search-item" @keydown.enter.native.prevent="getReviewList(1,formInline.size)" v-model="formInline.gcbh" placeholder="" />
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem label="工程名称" class="form-list-search" prop="gcmc">
              <Input type="text" class="form-search-item" @keydown.enter.native.prevent="getReviewList(1,formInline.size)" v-model="formInline.gcmc" placeholder="" />
            </FormItem>
          </i-col>
          <i-col span="6">
            <FormItem label="建设单位" class="form-list-search" prop="wtdw">
              <Input type="text" class="form-search-item" @keydown.enter.native.prevent="getReviewList(1,formInline.size)" v-model="formInline.wtdw" placeholder="" />
            </FormItem>
          </i-col>
        </Row>
        <Row>
          <i-col span="6">
            <FormItem label="测绘单位" class="form-list-search" prop="chdwmc">
              <Input type="text" class="form-search-item" @keydown.enter.native.prevent="getReviewList(1,formInline.size)" v-model="formInline.chdwmc" placeholder="" />
            </FormItem>
          </i-col>
          <i-col span="6">
    ...
    <Modal 
		...
		:footer-hide="true" 
		closable>
		<div class="margin-bottom-10 message-normal-tip" v-if="delFinishedData.length">
...
import { getDictInfo, getUploadList, saveBaxmSfgq } from "../../service/manage"
...
export default {
  	...
  	data() {
		...
		return {
			...
			tool: '<div>' +
					...
					'<span class="layui-btn main-btn-a" lay-event="btn3">批量打印回执单</span>' +
				'</div>',
			operationList: ["view","hangon","hangout"],
			selectedList: [],
			tableCols: [
				{
					type: "checkbox",
	...
	methods: {
		...
		handelClick(){
			...
			this.selectedList.forEach(item => {
				if(item.XMZT == "2"){
					...
					let params = {
						babh: item.BABH,
						xmid: this.createData.executionId,
						taskid: this.createData.taskId,
						gzlslid: this.createData.processInstanceId
					}
		...
		// 查看
		viewDetail(data) {
			this.$router.push({path: `/changzhou/htbadj/add`, query: {from: this.$route.fullPath,chxmid:data.CHXMID,slbh: data.SLBH,wtfs: data.XMLY}});
		},
		checkList(list){
		...
		hangon(data){
			...
			layer.confirm("确认挂起?", () => {
				this.$loading.show("挂起中...")
				saveBaxmSfgq(params).then(res => {
					this.$loading.close();
					layer.msg("挂起成功")
					this.getReviewList();
				})
		...
		deleteOpr(){
			...
			this.reviewList.forEach((list, index) => {
                if (list.SFGQ == "1") {
                    $(".layui-table:eq(1) tr:eq(" + index + ")").find("td").last().find("span[lay-event='hangon']").remove();
                } else {
					$(".layui-table:eq(1) tr:eq(" + index + ")").find("td").last().find("span[lay-event='hangout']").remove();
				}
            });
		},
		// 项目档案下载
		...
		printHzd(){
			...
			let chxmid = this.selectedList.map(select => select.CHXMID).join(";");
			if (!location.origin) {
				location.origin = location.protocol + "//" + location.hostname + (location.port ? ":" + location.port : "");
			}
			location.href = location.origin + "/msurveyplat-promanage/contractfile/exportPdf?chxmid=" + chxmid;
		},
		uploadCreate(data){
		...
		applyData(){
			...
			if(!this.selectedList.length){
				...
				initSqxx().then(res => {
					...
					let params = {
						ssmkid: this.ssmkid,
						glsxid: this.selectedList[0].BABH || ""
					}

...
import { queryCgFileList, getCgFileNums,queryDetailFile } from "../../../service/review"
export default {
    data() {
        return {
            backData: [],
    ...
    methods: {
        ...
        getTreeData(){
            ...
            queryCgFileList({chgcid: this.chgcid,chxmid: this.chxmid}).then(res => {
                this.$loading.close();
                this.sjr = res.data.dataList.author;
                this.treeData = res.data.dataList.items ? _.cloneDeep(res.data.dataList.items) : [];
        ...
        downloadFile(){
            ...
            if(select){
                ...
                getCgFileNums(params).then(res => {
                    location.href=location.origin + config.serviceOlContext + '/onlinedelegation/onlinegcdownload?wjzxid=' + select.id;
                })
            } else {
                layer.msg("暂无数据")
            }

	...
	<div>
		<div class="form-title mlk-top">
			...
			<div>
				...
				<Button class="btn-h-34 btn-cancel margin-left-10" v-if="!isRecord" @click="printHzd()">打印回执单</Button> -->
			</div>
		</div>
		<projectInfo 
			:readonly="readonly" 
		...
		<Modal 
			...
			:footer-hide="true" 
			closable>
			<Form v-if="visible" class="form-edit" ref="record-form" @on-validate="validateChecked" :model="recordForm" :rules="fileRule" :label-width="114">
	...
	methods: {
		...
		cgdownload() {
			getuploadfilenums({wjzxid: "", babh: this.babh}).then(res => {
				location.href=location.origin + config.msurveyplatContext + '/jcsjsq/achievementdownloadbybabh?babh=' + this.babh + "&cgccid=" + this.cgccid;
			})
		}
	},
};

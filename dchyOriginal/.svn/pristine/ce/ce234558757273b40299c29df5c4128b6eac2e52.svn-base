<template>
    <div class="evaluate-add">
        ...
        <Form class="form-edit" ref="evaluate-form" @on-validate="validateChecked" :model="evaluateForm" :rules="ruleInline" :label-width="114">
            ...
            <FormItem v-model="evaluateForm.ndkp"  prop="ndkp" label="考评年度 ">
                <Select filterable clearable v-model="evaluateForm.ndkp" style="width: 600px">
...
import { glryEvaludate, getKpnd, getNdkpList } from "../../../service/evaluate";
...
export default {
    ...
    methods: {
        getNdkpList(){
            getNdkpList().then(res => {
                this.dictInfo = res.data.datalist || [];
                this.getKpnd();
            })
        },
        // 获取已经考评的年度
        getKpnd(){
            ...
            getKpnd(params).then(res => {
                let kpndList = []
                if(res.data.ndkp&&res.data.ndkp.length){
                    this.dictInfo.forEach(zdlx => {
                        let dict = zdlx + '';
                        if(!res.data.ndkp.includes(dict)){
                            kpndList.push(zdlx)
                        }
                    })
                    this.kpndList = kpndList;
                } else {
                    this.kpndList = this.dictInfo;
                }
            })
        },
        initRate(){

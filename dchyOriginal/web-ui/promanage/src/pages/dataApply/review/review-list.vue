				...
				<Row>
					<i-col span="6">
						<FormItem label="受理编号" class="form-list-search" prop="slbh">
							<Input type="text" class="form-search-item" @keydown.enter.native.prevent="getReviewList(1,formInline.size)" v-model="formInline.slbh" placeholder="" />
						</FormItem>
					</i-col>
					<i-col span="6">
						<FormItem label="测绘单位" class="form-list-search" prop="chdw">
							<Input type="text" class="form-search-item" @keydown.enter.native.prevent="getReviewList(1,formInline.size)" v-model="formInline.chdw" placeholder="" />
						</FormItem>
					</i-col>
					<i-col span="6">
						<FormItem label="建设单位" class="form-list-search" prop="jsdw">
							<Input type="text" class="form-search-item" @keydown.enter.native.prevent="getReviewList(1,formInline.size)" v-model="formInline.jsdw" placeholder="" />
						</FormItem>
					</i-col>
					<i-col span="6">
...
import { deleteOtherAssignment } from "../../../service/manage";
export default {
	data() {
		return {
			formInline: {
	...
	methods: {
		...
		checkDetail(data) {
            ...
            deleteOtherAssignment({taskid: data.taskId,userid}).then(res => {
				this.$loading.close();
				let href = `/portal/view/service-page.html?taskid=${data.taskId}&processDefName=${data.text1}&chxmid=${data.CHXMID}&reviewType=verify&xmid=${data.text9}&gzlslid=${data.procInsId}&formKey=&wdid=${this.wdid}&type=db&ywlx=FCCH`
				window.open(href)
			});
		},
		// 制作
		dataMake(data) {
            ...
            deleteOtherAssignment({taskid: data.taskId,userid}).then(res => {
                this.$loading.close();
				let href = `/portal/view/service-page.html?taskid=${data.taskId}&processDefName=${data.text1}&chxmid=${data.CHXMID}&reviewType=make&xmid=${data.text9}&gzlslid=${data.procInsId}&formKey=&wdid=${this.wdid}&type=db&ywlx=FCCH`
				window.open(href)
			});
		},
		// 交付
		deliverDetail(data){
			if(!data.DQZT || data.DQZT == "已交付" || data.DQZT == "已退回"){
				return;
			}
			let userid = localStorage.getItem("userId") || ""
            deleteOtherAssignment({taskid: data.taskId,userid}).then(res => {
                this.$loading.close();
				let href = `/portal/view/service-page.html?taskid=${data.taskId}&processDefName=${data.text1}&chxmid=${data.CHXMID}&reviewType=dataApply&xmid=${data.text9}&gzlslid=${data.procInsId}&formKey=&wdid=${this.wdid}&type=db&ywlx=FCCH`
				window.open(href)
			});
		},
		// 查看

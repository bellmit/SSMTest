<template>
    <div class="submit-cg">
        ...
        <div v-if="errorData.length" class="line-dashed"></div>
        <div v-if="errorData.length" class="form-title margin-top-10">
            <div class="list-title">文件上传信息统计</div>
        </div>
        <div v-if="errorData.length" class="error-info-tj">
            <div style="width: 40%">
                <div id="error-info" style="width: 100%;height: 300px"> </div>
            </div>
        ...
        <div>
            <div class="error-header">
                <div>
                    <span v-if="hasError"><img src="static/images/access.png" style="margin-right: 10px;width:42px" alt=""><span style="font-size: 16px;">上传的文件检测出以下错误，是否确认提交？</span></span>
                    <span v-if="uploadFirst&&!hasError&&hasYxClsx"><img src="static/images/message-tip.png" style="margin-right: 10px;width:42px" alt=""><span style="font-size: 16px">成果包校验无误，是否确认提交?</span></span>
                    <span v-if="!hasYxClsx"><img src="static/images/access.png" style="margin-right: 10px;width:42px" alt=""><span style="font-size: 16px;">提交的成果都未备案，已为您直接过滤，是否重新上传?</span></span>
                </div>
                <div>
                    ...
                    <Button v-if="hasError" type="primary" class="bdc-major-btn margin-left-10" @click="exportBg">报告输出</Button>
            ...
            <div style="margin-top: 30px">
                <Form>
                    <FormItem :label-width="60" label="备注：">
                        <Input v-model="cgryBz" type="textarea"/>
                    </FormItem>
                </Form>
            </div>
            <div class="submit-back" >
                <Button type="primary" class="btn-h-34 bdc-major-btn" @click="transfer()">转发</Button>
...
import { checkZipFile, uploadZipFile, deleteSqxx, initCgtj } from "../../service/result-manage"
...
export default {
    ...
    mounted() {
        this.sqxxid = util.getSearchParams("xmid") || this.$route.query.xmid
    ...
    methods: {
        ...
        checkFileData(result){
            ...
            let errorData = [];
            msxxList.forEach(msxx => {
                let find = errorData.find(error => error.name == msxx.msxx)
                if(find){
                    let findIndex = errorData.findIndex(error => error.name == msxx.msxx)
                    errorData[findIndex].value += 1;
                } else {
                    errorData.push({
                        value: 1,
                        name: msxx.msxx
            ...
            if(errorData.length){
                this.initEcharts(errorData);
            }
            this.uploadErrorList = _.cloneDeep(msxxList)
        },
        initEcharts(data){
            this.$nextTick(() => {
                this.errorInfoEcharts = echarts.init(document.getElementById("error-info"));
                this.errorInfoOptions.series[0].data = data;
                this.errorInfoEcharts.setOption(this.errorInfoOptions);
            })
        },
        // 上传
        firstSubmit(){
            ...
            checkZipFile(formData).then(res => {
                ...
                if(wbaClsxList.length){
                    ...
                    layer.alert("测绘事项  " + wbaClsx + "  尚未备案，成果将被过滤", (index) => {
                        layer.close(index)
                        this.checkFileData(res.data.data)
                    })
                } else {
        ...
        exportBg(){
            ...
            let exportData = [];
            this.uploadErrorList.forEach(error => {
                exportData.push({clsx: error.clsx, wjmc: error.wjmc, mlmc: error.mlmc, msxx: error.msxx})
            })
            ...
            exportData.unshift({clsx: "测量事项", wjmc: "文件名称", mlmc: "目录名称", msxx: "描述信息"})
            LAY_EXCEL.exportExcel(exportData,"输出报告.xls","xls");
        }
    },
}
</script>

        ...
        <div class="search-form">
            <Form ref="formInline" :model="formInline" :rules="ruleInline" :label-width="120" inline>
                ...
                <Row>
                    ...
                    <i-col span="6">
                        <FormItem label="测绘阶段 " class="form-list-search" prop="ssjd">
                            <Select v-model="formInline.ssjd" clearable class="form-search-item">
                                <Option v-for="item in ssjdList" :value="item.dm" :key="item.mc">{{ item.mc }}</Option>
                            </Select>
                        </FormItem>
                    </i-col>
                    <i-col span="6">
...
<script>
import { getJsdwProjectList, querySsjd, jsdwHtDownload, onlinecompletecheck,onlinecomplete } from "../../../service/myproject"
...
export default {
    ...
    methods: {
        getSsjdList(){
            querySsjd().then(res => {
                this.ssjdList = res.data.dataList || []
            })
        },
        getDictInfo(){
            let params = {

...
import { onlinebafilepreview } from "../../../service/myproject"
import { queryFileList, getFileNums } from "../../../service/review"
export default {
    data() {
        return {
            backData: [],
    ...
    methods: {
        ...
        getTreeData(){
            ...
            if(this.$route.query.type == "xm"){
                let clsx = this.$route.query.clsx || "";
                this.clsxList = clsx.split(",")
                onlinebafilepreview({chxmid: this.$route.query.chxmid,clsxList: this.clsxList}).then(res => {
                    this.$loading.close();
        ...
        downloadFile(){
            ...
            if(select){
                ...
                getFileNums(params).then(res => {
                    ...
                    location.href=location.origin + '/msurveyplat-serviceol/fileoperation/download?wjzxid=' + select.wjzxid + "&cyryid=" + cyryid + "&ssmkid=" + ssmkid;
                })
            } else {
                layer.msg("暂无数据")
            }
        ...
        viewFile(file){
           if (!location.origin) {
            ...
            let href = location.origin + encodeURI('/msurveyplat-serviceol/fileoperation/onlinepreview?wjzxid=' + file.wjzxid + "&fileName=" + file.title);
            window.open(href);
        },
        // 查看
        viewDetail(){

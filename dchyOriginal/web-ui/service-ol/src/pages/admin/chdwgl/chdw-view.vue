    ...
    <div>
        <div class="form-title">
            ...
            <div>
                ...
                <Button class="btn-cancel btn-h-32 margin-left-10" @click="cancel">返回</Button>
            </div>
        </div>
        <div v-if="isCheck" class="form-edit">
            <Form :label-width="142">
                <FormItem label="注销原因 " >
                    <span  style="font-size: 16px;line-height: 36px;font-weight: bold;display: inline-block" class="font-color-tip">{{ycyy}}</span>
                </FormItem>
            </Form>
        </div>
        <agencyInfo
			...
			<Form v-if="visible" class="form-edit" ref="check-form" @on-validate="validateChecked"
				...
				<FormItem v-model="checkForm.shjg" prop="shjg" label="审核结果 ">
					<RadioGroup v-model="checkForm.shjg">
						<Radio label="98">通过</Radio>
						<Radio label="97">退回</Radio>
					</RadioGroup>
				</FormItem>
				<FormItem v-model="checkForm.shyj" prop="shyj" label="审核意见 ">
...
import { reviewCheck, queryYcyy, reviewData } from "../../../service/review"
export default {
    components: {
        agencyInfo,
        uploadFileInfo
    ...
    methods: {
        ...
        queryYcyy(){
            ...
            queryYcyy(params).then(res => {
                this.ycyy = res.data.ycyy || "";
            })
        },
        // 获取名录库信息
        getMlkInfo(){
        ...
        handlerCheck(){
            ...
            this.$refs["check-form"].validate(valid => {
                ...
                if(valid){
                    this.$loading.show("审核中...")
                    this.checkForm.mlkid = this.mlkid;
                    reviewData(this.checkForm).then(res => {
                        this.$loading.close();
                        this.cancel();
                    })
                }

        ...
        getMlkStatus(){
            ...
            getMlkStatus().then(res => {
                ...
                if(res.data.mlkid&&(res.data.sfyc=='是')){
                    ...
                    if(res.data.sfdj && res.data.sfshz == "是"){
                        // 跳到编辑页面
                        this.$router.push({path: "/survey/mlkapply", query: {mlkid: res.data.mlkid,blsx: res.data.blsx,ycyy: res.data.ycyy,ycsj: res.data.ycsj,toPath: this.from,type: 'draft'}})
                        sessionStorage.setItem("mlkid",res.data.mlkid || "")
                        sessionStorage.setItem("sqid",res.data.sqxxid || "")
                        sessionStorage.setItem("sfyx",res.data.sfyx || "")
                        sessionStorage.setItem("sfshz",res.data.sfshz || "")
                    }else if(res.data.mlkid){
                        // 跳到查看页面
                        this.$router.push({path: "/survey/application/view", query: {mlkid: res.data.mlkid,type:'view'}})
                        sessionStorage.setItem("mlkid",res.data.mlkid || "")
                        sessionStorage.setItem("sqid",res.data.sqxxid || "")

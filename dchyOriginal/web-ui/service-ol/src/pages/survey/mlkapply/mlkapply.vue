<template>
    <div>
        <div v-if="sfyc=='是'" class="line-height-32 apply-tip margin-bottom-20" >
            ...
            <div class="margin-left-20 font-size-16 sh-pass-tip">
        ...
        <div v-if="!shtg" class="line-height-32 apply-tip margin-bottom-20" >
            <div class="mlkrz-tip">
                <img src="static/images/message-tip.png" style="vertical-align:middle" alt="">
            </div>
            <div class="margin-left-20 font-size-16 sh-pass-tip">
                <div>您好，您的统一社会信用代码为：<span class="font-color-red">{{tyshxydm}}</span> 的测绘单位{{type == 'zx'? '注销':'移出名录库'}}审核中</div>
                <div><span>{{type == 'zx' ? '注销原因':'移出原因' }}：<span class="font-color-red">{{ycyy}}</span></span></div>
                <span>
                    <span>{{type == 'zx' ? '注销时间':'移出时间' }}：<span>{{ycsj}}</span></span>
                </span>
        ...
        <uploadFileInfo 
            ...
            :glsxid="mlkid" 
        ></uploadFileInfo>
        <div id="promise-btn" class="message-tip" v-if="flag=='true'&&shtg">
            <div class="list-tips">
                <span class="font-color-red">*</span><span class="font-color-major margin-left-10" style="font-size: 18px">申请人承诺</span>
            </div>  
            <div class="list-tips margin-left-20"> 
...
export default {
    ...
    created(){
        ...
        if(sessionStorage.getItem("sfshz")){
            this.shtg = sessionStorage.getItem("sfshz") == "是" ? false : true;
        }
        this.sfyc = this.$route.query.sfyc
        this.ycyy = this.$route.query.ycyy
        this.ycsj = this.$route.query.ycsj
        this.type = this.$route.query.blsx == "7" ? "yc": "zx"
        ...
        save(isSubmit){
            ...
            saveMlkData(this.agencyInfoData).then(res => {
                ...
                if(res.data){
                    ...
                    sessionStorage.removeItem("sfdj");
